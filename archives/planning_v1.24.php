...
function buildGrid(g){
  ...
  if(GROUPES_SPECIAUX.includes(g)){
    ...
    if(g==='lcl_parent'||g==='adc_lambert'){
      ...
      // GIE jours de semaine : 2 lignes de 3 colonnes
      // Ligne 1 : M - AM - J  |  Ligne 2 : P - R
      const gieOrder=['M','AM','J','P','R'];
      const orderedGie=gieOrder.map(code=>types.find(t=>t.code===code)).filter(Boolean);
      // Ajouter cellule vide pour compléter la 2ème ligne (P R _)
      if(orderedGie.length===5) orderedGie.push(null);
      types=orderedGie;
