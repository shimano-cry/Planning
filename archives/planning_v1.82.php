...
runMigrationOnce($mysqli, 'update_couleurs_conges_v3', function(mysqli $db) {
    ...
    // Absences spéciales
    $db->query("UPDATE types_conges SET couleur_bg='#FF00FF',couleur_txt='#000000' WHERE code='ASA'");
...
runMigrationOnce($mysqli, 'update_couleurs_conges_v4', function(mysqli $db) {
    ...
    $db->query("UPDATE types_conges SET couleur_bg='#ffcc80',couleur_txt='#7a3e00' WHERE code='PREV'");
});
...
runMigrationOnce($mysqli, 'update_couleurs_douane_v1', function(mysqli $db) {
    ...
    $db->query("UPDATE types_conges_douane SET libelle='Congés Annuels',              couleur_bg='#FF6633',couleur_txt='#000000' WHERE code='CA'");
...
$CODES_POLICE_DEFAUT = [
  ...
  'ASA' =>['code'=>'ASA', 'libelle'=>'Autorisation Spéciale d Absence',    'couleur_bg'=>'#FF00FF','couleur_txt'=>'#000000'],
...
$CODES_DOUANE_DEFAUT = [
  'J'   =>['code'=>'J',   'libelle'=>'Journée',                           'couleur_bg'=>'#FFFFFF','couleur_txt'=>'#000000'],
  ...
  'CA'  =>['code'=>'CA',  'libelle'=>'Congés Annuels',                    'couleur_bg'=>'#FF6633','couleur_txt'=>'#000000'],
  'CET' =>['code'=>'CET', 'libelle'=>'Compte Épargne Temps',              'couleur_bg'=>'#FF6633','couleur_txt'=>'#000000'],
  'NC'  =>['code'=>'NC',  'libelle'=>'Journée Non Cotée',                 'couleur_bg'=>'#FFCC99','couleur_txt'=>'#000000'],
  'CM'  =>['code'=>'CM',  'libelle'=>'Congé Maladie',                     'couleur_bg'=>'#FFCC99','couleur_txt'=>'#000000'],
  'GEM' =>['code'=>'GEM', 'libelle'=>'Garde Enfant Malade',               'couleur_bg'=>'#FF6633','couleur_txt'=>'#000000'],
  'AEA' =>['code'=>'AEA', 'libelle'=>'Autorisation Exceptionnelle Absence','couleur_bg'=>'#AECF00','couleur_txt'=>'#000000'],
  'RC'  =>['code'=>'RC',  'libelle'=>'Repos Compensatoire (Jour Férié)',   'couleur_bg'=>'#FF9966','couleur_txt'=>'#000000'],
...
.wrap-table td,.wrap-table th{width:38px;overflow:hidden}
.wrap-table td.agent-name,.wrap-table th:first-child{width:105px;min-width:105px}
...
td[data-groupe="douane"].j,td[data-groupe="douane_j"].j{background:#FFFFFF!important;color:#000!important}
td[data-groupe="douane"].m,td[data-groupe="douane_j"].m{background:#CCCCFF!important;color:#000!important}
td[data-groupe="douane"].am,td[data-groupe="douane_j"].am{background:#9999FF!important;color:#000!important}
...
td[data-groupe="lcl_parent"].j,td[data-groupe="adc_lambert"].j{background:#99CC00!important;color:#000!important}
...
td[data-groupe="lcl_parent"].rl,td[data-groupe="adc_lambert"].rl{background:#CCFFCC!important;color:#000!important}
...
.code-btn{border:2px solid transparent;border-radius:6px;padding:7px 4px;cursor:pointer;
  ...
  font-size:.76rem;line-height:1.3;transition:transform .1s,box-shadow .12s;text-align:center}
...
function openModal(td){
  ...
  // Exception : CoGe ROUSSEL peut modifier tous ses congés
  if(!IS_ADMIN && !IS_COGE && curCongeId>0){
...
function openCongeModal(td,date,groupe,cycle,isPerm){
  ...
  if(isPerm){
    ...
    const GROUPES_VAC_ONLY=['direction_police','direction_mokadem'];
    const GROUPES_VAC_INDISPO=['standard_j','nuit'];
    const labelPerm=GROUPES_VAC_ONLY.includes(groupe)?'choisir la vacation'
      :GROUPES_VAC_INDISPO.includes(groupe)?'choisir la vacation ou l\'indisponibilit\u00e9'
      :'choisir une permanence';
    mCycle.innerHTML='Cycle d\'origine : <strong>'+curCycleOrig+'</strong> \u2192 '+labelPerm;
    hideSections();secPerm.style.display='block';

    const isGieGroupe  = (groupe==='gie'||groupe==='gie_j');
    const isNuitGroupe = (groupe==='nuit');
    const isEquipeGroupe = (groupe==='equipe'||groupe==='standard_police');
    const isDirectionGroupe = (groupe==='direction_police'||groupe==='direction_mokadem');
    const isCoGeOnWE = IS_COGE && isDirectionGroupe; // CoGe ROUSSEL : vacation étendue RC/RL/FERIE
    // Vacations M/AM réservées admin pour équipes 1&2 et GIE ; toujours visibles pour nuit et direction (admin ou non)
    const showVacMAM = isNuitGroupe || isDirectionGroupe || isCoGeOnWE || (IS_ADMIN && (isEquipeGroupe||isGieGroupe));
    // Bouton Nuit : uniquement groupe nuit et direction (+ CoGe)
    const btnPermNuit=document.getElementById('perm-nuit');
    if(btnPermNuit) btnPermNuit.style.display=((isNuitGroupe||isDirectionGroupe||isCoGeOnWE)&&showVacMAM)?'inline-block':'none';
    // Bouton J : uniquement CoGe ROUSSEL
    let btnPermJ=document.getElementById('perm-j');
    if(!btnPermJ && isCoGeOnWE){
      // Créer le bouton J à la volée s'il n'existe pas
      btnPermJ=document.createElement('button');
      btnPermJ.type='button'; btnPermJ.id='perm-j';
      btnPermJ.className='perm-btn';
      btnPermJ.style.cssText='background:#ffa000;color:#fff';
      btnPermJ.innerHTML='J<small>Vacation Journ\u00e9e</small>';
      btnPermJ.addEventListener('click',()=>{
        selType='J';
        document.querySelectorAll('.perm-btn').forEach(b=>b.classList.remove('sel'));
        btnPermJ.classList.add('sel'); btnSave.disabled=false;
      });
      document.getElementById('perm-grid').appendChild(btnPermJ);
    }
    if(btnPermJ) btnPermJ.style.display=isCoGeOnWE?'inline-block':'none';

    document.getElementById('perm-m').style.display=showVacMAM?'inline-block':'none';
    document.getElementById('perm-am').style.display=showVacMAM?'inline-block':'none';
    // Indisponibilités : masquées pour direction et CoGe
    ['perm-indispo-m','perm-indispo-am','perm-indispo-j'].forEach(id=>{
      const el=document.getElementById(id);
      if(el) el.style.display=(isDirectionGroupe||isCoGeOnWE)?'none':'inline-block';
    });
    ...
    if(permGrid){
      if(isCoGeOnWE){
        // CoGe : M AM NUIT J sur une ligne (4 colonnes)
        permGrid.style.gridTemplateColumns='repeat(4,1fr)';
      ...
      } else if((isNuitGroupe||isDirectionGroupe) && showVacMAM){
        // NUIT / DIRECTION : ligne 1 = M AM NUIT (3 colonnes égales), pas d'indispos pour direction
        permGrid.style.gridTemplateColumns='repeat(3,1fr)';
        modal.style.width='560px';
        ['perm-m','perm-am','perm-nuit','perm-indispo-m','perm-indispo-am','perm-indispo-j'].forEach(id=>{
          const el=document.getElementById(id); if(el) el.style.gridColumn='span 1';
        });
      } else if(showVacMAM){
        // ÉQUIPE / GIE (admin) : ligne 1 = M AM (6 cols : span3+span3) / ligne 2 = IM IAM IJ (span2 each)
        ...
        // Pas de vacations : uniquement 3 indispos sur une ligne
        permGrid.style.gridTemplateColumns='repeat(3,1fr)';
    ...
    if(selType==='NUIT') document.getElementById('perm-nuit')?.classList.add('sel');
    if(selType==='J')    document.getElementById('perm-j')?.classList.add('sel');
  ...
  ensureQuotaCache(); // Précharger quota en arrière-plan
    ...
    const canDelConge=IS_ADMIN||(curCongeId===0)||_userAllowed.includes(congeTypeExist);
    btnDel.style.display=(!IS_ADMIN&&curCongeId>0&&!_userAllowed.includes(congeTypeExist))?'none':'';
...
function buildGrid(g){
  ...
  if(GROUPES_SPECIAUX.includes(g)){
    ...
    types=ordered;
    codeGrid.style.gridTemplateColumns='repeat(3,1fr)';
    document.querySelector('.modal').style.width='480px';
  } else {
...
function buildVacOverrideSection(groupe, currentVac, vacId){
  ...
  if(!IS_ADMIN && !IS_COGE && !isDouane){ secVacOverride.style.display='none'; return; }
