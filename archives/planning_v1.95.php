...
runMigrationOnce($mysqli, 'update_couleurs_douane_v3', function(mysqli $db) {
    ...
    $db->query("UPDATE types_conges_douane SET couleur_bg='#ffcc99',couleur_txt='#ffffff'
                WHERE code IN ('CET','NC','CM','GEM','AEA','RC','RH','DA','PR')");
});
        ...
        foreach ($datesList as $dt) {
            ...
            if ($annId > 0) {
                ...
                if ($cgCode === 'PREV') {
                    ...
                    $styleCg = " style='background:$bgCg;color:$txtCg;font-weight:bold'";
                }
...
$CODES_DOUANE_DEFAUT = [
  ...
  'CET' =>['code'=>'CET', 'libelle'=>'Compte Épargne Temps',              'couleur_bg'=>'#ffcc99','couleur_txt'=>'#ffffff'],
  'NC'  =>['code'=>'NC',  'libelle'=>'Journée Non Cotée',                 'couleur_bg'=>'#ffcc99','couleur_txt'=>'#ffffff'],
  'CM'  =>['code'=>'CM',  'libelle'=>'Congé Maladie',                     'couleur_bg'=>'#ffcc99','couleur_txt'=>'#ffffff'],
  'GEM' =>['code'=>'GEM', 'libelle'=>'Garde Enfant Malade',               'couleur_bg'=>'#ffcc99','couleur_txt'=>'#ffffff'],
  'AEA' =>['code'=>'AEA', 'libelle'=>'Autorisation Exceptionnelle Absence','couleur_bg'=>'#ffcc99','couleur_txt'=>'#ffffff'],
  'RC'  =>['code'=>'RC',  'libelle'=>'Repos Compensatoire (Jour Férié)',   'couleur_bg'=>'#ffcc99','couleur_txt'=>'#ffffff'],
  'RH'  =>['code'=>'RH',  'libelle'=>'Repos Hebdomadaire',                'couleur_bg'=>'#ffcc99','couleur_txt'=>'#ffffff'],
  'DA'  =>['code'=>'DA',  'libelle'=>'Départ avancé',                     'couleur_bg'=>'#ffcc99','couleur_txt'=>'#ffffff'],
  'PR'  =>['code'=>'PR',  'libelle'=>'Prise retardée',                    'couleur_bg'=>'#ffcc99','couleur_txt'=>'#ffffff'],
];
  ...
  for ($d=1;$d<=$nbJours;$d++) {
    ...
    if ($tirPer !== null) {
      ...
      if ($cgCode === 'PREV') {
          ...
          $styleCg2 = " style='background:$bgCg2;color:$txtCg2;font-weight:bold'";
      }
...
.wrap-table td,.wrap-table th{width:42px;overflow:hidden;text-overflow:ellipsis}
...
function openCongeModal(td,date,groupe,cycle,isPerm){
  ...
  if(isPerm){
    ...
    // NUIT et DIRECTION : les vacations sont dans "Modifier la vacation affichée" → on masque les boutons
    const showVacMAM = !isNuitGroupe && !isDirectionGroupe && !isCoGeOnWE && (IS_ADMIN && (isEquipeGroupe||isGieGroupe));
    // Bouton Nuit : masqué pour tous (géré via vacation override)
    const btnPermNuit=document.getElementById('perm-nuit');
    if(btnPermNuit) btnPermNuit.style.display='none';
    // Bouton J : masqué (géré via vacation override pour CoGe)
    let btnPermJ=document.getElementById('perm-j');
    if(btnPermJ) btnPermJ.style.display='none';
    ...
    // Indisponibilités : masquées pour direction et CoGe, visibles pour nuit
    ...
    if(permGrid){
      if(isNuitGroupe){
        // NUIT : uniquement 3 indispos (les vacations sont dans "Modifier la vacation")
        permGrid.style.gridTemplateColumns='repeat(3,1fr)';
        modal.style.width='480px';
        ['perm-indispo-m','perm-indispo-am','perm-indispo-j'].forEach(id=>{
          const el=document.getElementById(id); if(el) el.style.gridColumn='span 1';
        });
      } else if(isDirectionGroupe||isCoGeOnWE){
        // DIRECTION/CoGe : pas d'indispos ni vacations → grille vide (section vacation override suffit)
        permGrid.style.gridTemplateColumns='repeat(3,1fr)';
        modal.style.width='480px';
      } else if(showVacMAM){
        // ÉQUIPE / GIE (admin) : ligne 1 = M AM (6 cols) / ligne 2 = IM IAM IJ
    ...
    // Section "Modifier la vacation affichée" pour nuit et direction
  ...
  ensureQuotaCache(); // Précharger quota en arrière-plan
    ...
    const canDelConge=IS_ADMIN||IS_DOUANE||(curCongeId===0)||_userAllowed.includes(congeTypeExist);
    btnDel.style.display=(!IS_ADMIN&&!IS_DOUANE&&curCongeId>0&&!_userAllowed.includes(congeTypeExist))?'none':'';
...
function getVacOptions(groupe){
  ...
  if(groupe==='direction_police'||groupe==='direction_mokadem') return [
    {val:'M',   label:'✳ Matin'  },
    {val:'AM',  label:'☽ AM'     },
    {val:'NUIT',label:'🌙 Nuit'   },
    {val:'J',   label:'⊙ Journée'},
