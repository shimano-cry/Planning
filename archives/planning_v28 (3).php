...
runMigrationOnce($mysqli, 'alter_permanences_type_v1', function(mysqli $db) {
    // Ignorer si la table n'existe pas encore (install_planning.sql pas encore exécuté)
    try {
    ...
    } catch (Throwable $e) { /* table absente, sera créée par install_planning.sql */ }
});
...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['action_auth'])) {
    ...
    if ($act==='forgot_password') {
        ...
        try {
            ...
            $resetMsg = '❌ Base de données non initialisée. Veuillez exécuter install_planning.sql.';
        }
    } elseif ($act==='login') {
        $login = trim($_POST['login'] ?? '');
        $pass  = $_POST['password'] ?? '';
        try {
            ...
            $loginError = "Base de données non initialisée. Veuillez exécuter install_planning.sql.";
        }
    } elseif ($act==='logout') {
        session_destroy();
        header('Location: '.$_SERVER['PHP_SELF']); exit;
...
function canEdit(string $agent, bool $isAdmin, bool $globalLocked, array $locks, array $userAgents): bool {
...
/* Logo miniature centré dans la carte */
.login-logo{
  display:block;
  width:72px;height:72px;
  margin:0 auto 14px;
  border-radius:50%;
  box-shadow:0 2px 12px rgba(13,36,97,.25);
}
...
<div class="login-wrap">
  ...
  <!-- Carte connexion -->
  <div class="login-box">
    <img class="login-logo" src="Logo_CCPD.png" alt="Logo CCPD" onerror="this.style.display='none'">
    <h1>Connexion</h1>
    ...
    <div id="form-login">
      <form method="POST">
        ...
        <input type="text" name="login" autofocus autocomplete="username" required placeholder="Votre identifiant">
  ...
  <div class="login-version">Planning v20 &mdash; C.C.P.D. Tournai</div>
</div>
...
$mois  = isset($_GET['mois'])  ? max(1,min(12,(int)$_GET['mois'])) : 1;
$vue   = $_GET['vue'] ?? 'mois';
...
function renderCellsHtml(mysqli $db, array $agentsList, array $datesList,
    ...
    while ($c = $res->fetch_assoc()) {
        ...
        while ($d <= $fi) {
            ...
            if (in_array($ds, $datesList)) {
                ...
                $cgId[$a][$ds]   = $c['id'];
                $cgMeta[$a][$ds] = ['periode'=>$c['periode']??'J','libelle'=>$c['libelle'],'heure'=>$c['heure']??''];
            }
            $d->modify('+1 day');
        }
    }
    ...
    foreach ($agentsList as $ag) {
        ...
        foreach ($datesList as $dt) {
            ...
            $lockCls  = (!$isAdmin && !canEdit($ag,$isAdmin,$globalLocked,$locks,$userAgents)) ? ' locked-agent' : '';
            ...
            if ($annId > 0) {
                ...
                if ($cgCode && in_array($ag, $agentsCombine)) {
                    $meta   = $cgMeta[$ag][$dt] ?? ['periode'=>'J','libelle'=>$cgCode,'heure'=>''];
                    $per    = $meta['periode'] ?? 'J';
                    $hre    = $meta['heure']   ?? '';
                    ...
                    echo "<td class='tir tir-ok conge$lockCls'$da data-ferie='0' data-conge-id='$cgId2' data-conge-type='$cgCode' data-perm-id='0' data-tir-id='$tirId2' data-tir-per='$tirPer' data-conge-per='$per' data-conge-heure='".htmlspecialchars($hre)."' data-annul-id='0' title='".htmlspecialchars($tip)."' style='font-size:7px'>$txt</td>";
                } else {
                ...
                echo "<td class='conge$lockCls'$da data-ferie='0' data-conge-id='$cgId2' data-conge-type='$cgCode' data-perm-id='0' data-tir-id='0' data-conge-per='$per' data-conge-heure='".htmlspecialchars($hre)."' title='".htmlspecialchars($tip)."'>$txt</td>";
...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_SERVER['HTTP_X_REQUESTED_WITH'])) {
    ...
    /* ── Changer son propre mot de passe ── */
        ...
        if (!$isAdmin && !($userRights['can_conge']??false)) { echo json_encode(['ok'=>false,'msg'=>'Droits insuffisants']); exit; }
        ...
        // Retourne les quotas par type pour les agents Police uniquement
        if (!$isAdmin) { echo json_encode(['ok'=>false]); exit; }
        ...
        // Renvoyer uniquement les agents police
        $result = [];
        ...
        // Compte les congés posés PAR TYPE (hors WE et fériés) — version optimisée SQL
        $yr = (int)($_POST['annee'] ?? date('Y'));

        // 1. Fériés de l'année (petit set, rapide)
        $feriesAnnee = [];
        ...
        $nbFeries = count($feriesAnnee);

        // 2. Compter les jours ouvrés par congé via SQL :
        //    jours_calendaires - samedis - dimanches dans la plage
        //    puis soustraire les fériés tombant en semaine dans la plage côté PHP
        $yr1 = $yr.'-01-01';
        $yr2s = $yr.'-12-31';
        $stC = $mysqli->prepare(
            "SELECT agent, type_conge,
                    GREATEST(date_debut, ?) AS d1,
                    LEAST(date_fin, ?)    AS d2
             FROM conges
             WHERE date_debut <= ? AND date_fin >= ?
             ORDER BY agent, type_conge"
        );
        $stC->bind_param('ssss', $yr1, $yr2s, $yr2s, $yr1);
        $stC->execute();
        $resC = $stC->get_result();
        ...
        while ($c = $resC->fetch_assoc()) {
            ...
            $type = $c['type_conge'];
            $d    = new DateTime($c['d1']);
            $fin  = new DateTime($c['d2']);

            // Nombre de jours calendaires dans la plage (inclus)
            $total = (int)$d->diff($fin)->days + 1;

            // Soustraire samedis et dimanches
            // Semaines complètes × 2 WE + WE partiels
            $dow1 = (int)$d->format('N');   // 1=lun, 7=dim
            $dow2 = (int)$fin->format('N');
            $fullWeeks = intdiv($total, 7);
            $rem = $total % 7;
            $we = $fullWeeks * 2;
            // Jours restants : compter sam(6)/dim(7) dans [dow1 .. dow1+rem-1]
            for ($i = 0; $i < $rem; $i++) {
                $d_ = (($dow1 - 1 + $i) % 7) + 1;
                if ($d_ >= 6) $we++;
            }
            ...
            // Soustraire les fériés tombant en semaine dans la plage
            foreach ($feriesAnnee as $fDate => $_) {
                if ($fDate >= $c['d1'] && $fDate <= $c['d2']) {
                    $dowF = (int)(new DateTime($fDate))->format('N');
                    if ($dowF < 6) $ouvrables--;
                }
            }

            if ($ouvrables > 0) {
                $counts[$ag][$type] = ($counts[$ag][$type] ?? 0) + $ouvrables;
            }
        }
        echo json_encode(['ok'=>true,'counts'=>$counts,'annee'=>$yr]);
        exit;
    ...
    if (in_array($action,['save_conge','delete_conge','save_perm','delete_perm','save_tir','delete_tir'])) {
        if (!canEdit($agent,$isAdmin,$globalLocked,$locks,$userAgents)) {
        ...
        if (in_array($action,['save_conge','delete_conge']) && !$isAdmin && !($userRights['can_conge']??false)) {
    ...
    if ($action==='save_conge') {
        ...
        $r1=$mysqli->query("SELECT MAX(id) as m FROM permanences");$r2=$mysqli->query("SELECT MAX(id) as m FROM conges");
        $tk=($r1&&$rk=$r1->fetch_assoc()?(int)$rk['m']:0).'-'.($r2&&$rk=$r2->fetch_assoc()?(int)$rk['m']:0);
        echo json_encode(['ok'=>true,'msg'=>'Congé sauvegardé','id'=>$nid,'date_debut'=>$date_debut,'date_fin'=>$date_fin,'cells'=>$cells,'token'=>$tk]);
    ...
    /* ── Suppression CONGÉ ── */
        ...
        $r1=$mysqli->query("SELECT MAX(id) as m FROM permanences");$r2=$mysqli->query("SELECT MAX(id) as m FROM conges");
        $tk=($r1&&$rk=$r1->fetch_assoc()?(int)$rk['m']:0).'-'.($r2&&$rk=$r2->fetch_assoc()?(int)$rk['m']:0);
        echo json_encode(['ok'=>$ok,'msg'=>$ok?'Congé supprimé':'Erreur : '.$mysqli->error,'cells'=>$cells,'token'=>$tk]);
    ...
    /* ── Sauvegarde PERMANENCE ── */
        ...
        $r1=$mysqli->query("SELECT MAX(id) as m FROM permanences");$r2=$mysqli->query("SELECT MAX(id) as m FROM conges");
        $tk=($r1&&$rk=$r1->fetch_assoc()?(int)$rk['m']:0).'-'.($r2&&$rk=$r2->fetch_assoc()?(int)$rk['m']:0);
        echo json_encode(['ok'=>true,'msg'=>'Permanence sauvegardée','id'=>$nid,'cells'=>$cells,'token'=>$tk]);
    ...
    /* ── Suppression PERMANENCE ── */
        ...
        $r1=$mysqli->query("SELECT MAX(id) as m FROM permanences");$r2=$mysqli->query("SELECT MAX(id) as m FROM conges");
        $tk=($r1&&$rk=$r1->fetch_assoc()?(int)$rk['m']:0).'-'.($r2&&$rk=$r2->fetch_assoc()?(int)$rk['m']:0);
        echo json_encode(['ok'=>$ok,'msg'=>$ok?'Permanence supprimée':'Erreur : '.$mysqli->error,'cells'=>$cells,'token'=>$tk]);
    ...
    /* ── Sauvegarde TIR ── */
        ...
        $r1=$mysqli->query("SELECT MAX(id) as m FROM permanences");$r2=$mysqli->query("SELECT MAX(id) as m FROM conges");
        $tk=($r1&&$rk=$r1->fetch_assoc()?(int)$rk['m']:0).'-'.($r2&&$rk=$r2->fetch_assoc()?(int)$rk['m']:0);
        echo json_encode(['ok'=>$ok,'msg'=>$ok?'TIR sauvegardé':'Erreur : '.$mysqli->error,'id'=>$nid,'cells'=>$cells,'token'=>$tk]);
    ...
    /* ── Suppression TIR ── */
        ...
        $r1=$mysqli->query("SELECT MAX(id) as m FROM permanences");$r2=$mysqli->query("SELECT MAX(id) as m FROM conges");
        $tk=($r1&&$rk=$r1->fetch_assoc()?(int)$rk['m']:0).'-'.($r2&&$rk=$r2->fetch_assoc()?(int)$rk['m']:0);
        echo json_encode(['ok'=>$ok,'msg'=>$ok?'TIR supprimé':'Erreur : '.$mysqli->error,'cells'=>$cells,'token'=>$tk]);
    ...
    /* ── Annulation TIR ── */
        ...
        $r1=$mysqli->query("SELECT MAX(id) as m FROM permanences");$r2=$mysqli->query("SELECT MAX(id) as m FROM conges");
        $tk=($r1&&$rk=$r1->fetch_assoc()?(int)$rk['m']:0).'-'.($r2&&$rk=$r2->fetch_assoc()?(int)$rk['m']:0);
        echo json_encode(['ok'=>$ok,'id'=>$nid,'msg'=>$ok?'Annulation enregistrée':'Erreur : '.$mysqli->error,'cells'=>$cells,'token'=>$tk]);
        ...
        $r1=$mysqli->query("SELECT MAX(id) as m FROM permanences");$r2=$mysqli->query("SELECT MAX(id) as m FROM conges");
        $tk=($r1&&$rk=$r1->fetch_assoc()?(int)$rk['m']:0).'-'.($r2&&$rk=$r2->fetch_assoc()?(int)$rk['m']:0);
        echo json_encode(['ok'=>$ok,'msg'=>$ok?'Annulation supprimée':'Erreur','cells'=>$cells,'token'=>$tk]);
    ...
    /* ── Compteur TIR ── */
        ...
        echo json_encode(['ok'=>true,'annuel'=>$annuel,'parMois'=>$parMois,'quadri'=>$quadri,
                          ...
                          'annulations'=>$annulations,'annee'=>$yr,'mois'=>$mo,'quad'=>$qr]);
...
function loadConges(mysqli $db,int $annee,int $moisD,int $moisF): array {
    ...
    while ($c=$res->fetch_assoc()) {
        ...
        while ($d<=$fi) {
            ...
            $congesMeta[$ag][$ds]= ['periode'=>$c['periode']??'J','libelle'=>$c['libelle'],'heure'=>$c['heure']??''];
            $d->modify('+1 day');
        }
    }
...
$CODES_POLICE_DEFAUT = [
  'CA'  =>['code'=>'CA',  'libelle'=>'Congé annuel',             'couleur_bg'=>'#7030a0','couleur_txt'=>'#ffffff'],
  'CAA' =>['code'=>'CAA', 'libelle'=>'Congé annuel anticipé',    'couleur_bg'=>'#7030a0','couleur_txt'=>'#ffffff'],
  'CAM' =>['code'=>'CAM', 'libelle'=>'Congé annuel maintenu',    'couleur_bg'=>'#7030a0','couleur_txt'=>'#ffffff'],
  ...
  'DA'  =>['code'=>'DA',  'libelle'=>'Départ anticipé',          'couleur_bg'=>'#fce5cd','couleur_txt'=>'#222222'],
  'PR'  =>['code'=>'PR',  'libelle'=>'Prise service retardée',   'couleur_bg'=>'#fce5cd','couleur_txt'=>'#222222'],
  'HP'  =>['code'=>'HP',  'libelle'=>'Heure de préparation',     'couleur_bg'=>'#d9d2e9','couleur_txt'=>'#222222'],
  'HPA' =>['code'=>'HPA', 'libelle'=>'Heure préparation AM',     'couleur_bg'=>'#d9d2e9','couleur_txt'=>'#222222'],
  'HS'  =>['code'=>'HS',  'libelle'=>'Heure supplémentaire',     'couleur_bg'=>'#f4cccc','couleur_txt'=>'#222222'],
  'CET' =>['code'=>'CET', 'libelle'=>'Compte épargne-temps',     'couleur_bg'=>'#674ea7','couleur_txt'=>'#ffffff'],
  'CF'  =>['code'=>'CF',  'libelle'=>'Congé de formation',       'couleur_bg'=>'#e6b8a2','couleur_txt'=>'#222222'],
  'RPS' =>['code'=>'RPS', 'libelle'=>'Risques psycho-sociaux',   'couleur_bg'=>'#ead1dc','couleur_txt'=>'#222222'],
  'ASA' =>['code'=>'ASA', 'libelle'=>'Absence service autorisée','couleur_bg'=>'#a4c2f4','couleur_txt'=>'#1a237e'],
  'CONV'=>['code'=>'CONV','libelle'=>'Convocation officielle',   'couleur_bg'=>'#ffe599','couleur_txt'=>'#222222'],
  'GEM' =>['code'=>'GEM', 'libelle'=>'Garde enfant malade',      'couleur_bg'=>'#d9ead3','couleur_txt'=>'#222222'],
  'VEM' =>['code'=>'VEM', 'libelle'=>'Visite emploi maintenu',   'couleur_bg'=>'#d0e0e3','couleur_txt'=>'#222222'],
  'STG' =>['code'=>'STG', 'libelle'=>'Stage',                    'couleur_bg'=>'#cfe2f3','couleur_txt'=>'#222222'],
  'RCH' =>['code'=>'RCH', 'libelle'=>'Récupération chef',        'couleur_bg'=>'#b6d7a8','couleur_txt'=>'#222222'],
  'RCR' =>['code'=>'RCR', 'libelle'=>'Récupération commandant',  'couleur_bg'=>'#b6d7a8','couleur_txt'=>'#222222'],
...
$codesLclParent=['P'=>['code'=>'P','libelle'=>'Permission','couleur_bg'=>'#7030a0','couleur_txt'=>'#ffffff']];

// Codes ADC LAMBERT (Sous-officier GIE) : Permission + Repos (Matin/AM/Journée)
$codesAdcLambert=[
  'P'=>['code'=>'P','libelle'=>'Permission','couleur_bg'=>'#7030a0','couleur_txt'=>'#ffffff'],
...
function ligne(string $agent,callable $cb,bool $masqueRC,bool $ignFe=false,string $grp='standard'): void {
  global $annee,$mois,$nbJours,$conges,$congesIds,$congesMeta,$perms,$permsIds,$feries,$agentsPermanence,$tirs,$tirsIds,$tirsAnnul,$agentsTir,$isAdmin,$globalLocked,$locks,$userAgents,$symSoleil,$svgIndispo,$vacOvr,$vacOvrId;
  $pP      = in_array($agent,$agentsPermanence);
  $pTir    = in_array($agent,(array)$agentsTir);
  $editable= canEdit($agent,$isAdmin,$globalLocked,$locks,$userAgents);
  $lockCls = $editable ? '' : ' locked-agent';
  $lockName= $editable ? '' : ' locked-name';
  echo "<tr><td class='agent-name$lockName'>".htmlspecialchars($agent)."</td>";
  for ($d=1;$d<=$nbJours;$d++) {
    ...
    if ($tirPer !== null) {
      ...
      if ($cgCode && in_array($agent,$agentsCombine)) {
        ...
        $meta  = $congesMeta[$agent][$date] ?? ['periode'=>'J','libelle'=>$cgCode,'heure'=>''];
        $per   = $meta['periode']??'J';
        $hre   = $meta['heure']??'';
        $symCg = match($per){'M'=>'🔆','AM'=>'🌙',default=>''};
        ...
        $txt   = 'TIR'.$symTir.'+'.$cgCode.$symCg;
        echo "<td class='tir tir-ok conge$lockCls'$da data-ferie='0' data-conge-id='$cgId' data-conge-type='$cgCode' data-perm-id='0' data-tir-id='$tirId' data-tir-per='$tirPer' data-conge-per='$per' data-conge-heure='".htmlspecialchars($hre)."' data-annul-id='0' title='".htmlspecialchars($tip)."' style='font-size:7px'>$txt</td>";
      } else {
      ...
      if($hre && in_array($cgCode,['DA','PR'])) $tip .= ' à '.substr($hre,0,5);
      if($per!=='J') $tip .= ' ('.match($per){'M'=>'Matin','AM'=>'Après-midi',default=>$per}.')';
      echo "<td class='conge$lockCls'$da data-ferie='0' data-conge-id='$cgId' data-conge-type='$cgCode' data-perm-id='0' data-tir-id='0' data-conge-per='$per' data-conge-heure='".htmlspecialchars($hre)."' title='".htmlspecialchars($tip)."'>$cellTxt</td>";
...
.leg-conge{background:#7030a0;color:#fff}
...
/* ── PANNEAU NOTES ── */
.notes-panel{
  width:440px;min-width:400px;flex-shrink:0;
...
table{border-collapse:collapse;font-size:10.5px;background:#fff;
  box-shadow:0 1px 8px rgba(0,0,0,.1)}
td,th{border:1px solid #ccc;padding:2px 4px;text-align:center;white-space:nowrap}
th{background:#1a2742;color:#fff;font-weight:600;font-size:10px;position:sticky;top:0;z-index:2}
th.wk{background:#253560}
th.ferie-th{background:#9a7200;color:#fff}

td.agent-name{
  text-align:left;padding:3px 7px;font-weight:600;background:#f7f8fa;
  min-width:120px;position:sticky;left:0;z-index:1;
  border-right:2px solid #bbb;cursor:default;font-size:10.5px;
}
td.equipe{
...
td.locked-agent{opacity:.7;cursor:not-allowed!important}
td.locked-agent:hover{filter:none!important;transform:none!important}
.agent-name.locked-name::after{content:' 🔒';font-size:.7rem}
/* Panneau admin */
.overlay-admin{display:none;position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:300;align-items:flex-start;justify-content:center;padding:20px;overflow-y:auto}
.overlay-admin.open{display:flex}
.modal-admin{background:#fff;border-radius:12px;width:100%;max-width:820px;box-shadow:0 8px 32px rgba(0,0,0,.25);margin:auto}
.adm-head{background:#1a2742;color:#fff;padding:12px 18px;border-radius:12px 12px 0 0;display:flex;align-items:center;justify-content:space-between}
.adm-head h3{font-size:.95rem;font-weight:700}
.adm-tabs{display:flex;border-bottom:2px solid #eee;padding:0 16px}
...
.adm-body{padding:16px;max-height:70vh;overflow-y:auto}
...
.mini-cal .ferie{background:#ffeb3b;color:#c00000}.mini-cal .conge{background:#7030a0;color:#fff}
...
@media print {
  ...
  .conge{background:#7030a0!important;color:#fff!important;-webkit-print-color-adjust:exact;print-color-adjust:exact}
...
<div class="mini-mois">
  ...
  <tr>
    ...
    <?php for($d=1;$d<=$nb;$d++):
      ...
      elseif($cg) { $lc=canEdit($agent,$isAdmin,$globalLocked,$locks,$userAgents)?'':' locked-agent'; echo "<td class='conge$lc'>•</td>"; }
  ...
  <div class="modal">
    ...
    <div class="modal-body">
      ...
      <div id="sec-vac-override" style="display:none;margin:8px 16px;padding:9px 12px;background:#fff8e1;border:1.5px solid #f9a825;border-radius:8px">
        ...
        <div style="display:flex;gap:6px;flex-wrap:wrap" id="vac-btns"></div>
  ...
  <div class="modal-admin">
    ...
    <div class="adm-body">
      ...
      <div class="adm-section active" id="tab-verrous">
        ...
        <p style="font-size:.78rem;color:#888;margin-bottom:10px">Verrouillage par agent (cliquer pour basculer) :</p>
        <div id="lock-agents-list">
          <?php
          ...
          foreach($tousAgents as $ag):
            $isLocked=isset($locks['agents'][$ag]);
          ?>
          <div class="lock-row <?=$isLocked?'locked':'unlocked'?>" data-agent="<?=htmlspecialchars($ag)?>">
            <span style="flex:1;font-weight:600"><?=htmlspecialchars($ag)?></span>
            <span style="font-size:.75rem;color:<?=$isLocked?'#c0392b':'#27ae60'?>"><?=$isLocked?'🔒 Verrouillé':'🔓 Libre'?></span>
            <button class="btn-action btn-toggle-lock" data-agent="<?=htmlspecialchars($ag)?>" data-locked="<?=$isLocked?1:0?>"
              style="padding:3px 10px;font-size:.72rem;background:<?=$isLocked?'#27ae60':'#c0392b'?>;color:#fff;border:none;border-radius:5px;cursor:pointer">
      ...
      <div class="adm-section" id="tab-users">
        ...
        <!-- Formulaire user (caché par défaut) -->
        <div id="user-form-wrap" style="display:none;margin-top:14px;background:#f7f8fa;padding:14px;border-radius:8px">
          <h4 style="font-size:.82rem;font-weight:700;margin-bottom:10px" id="user-form-title">Nouvel utilisateur</h4>
          <div class="adm-form">
            ...
            <div style="display:flex;gap:8px;margin-top:4px">
              <button class="btn-action btn-print" id="btn-save-user">Enregistrer</button>
              <button class="btn-action" id="btn-cancel-user" style="background:#eee;color:#333">Annuler</button>
            </div>
          </div>
        </div>
      </div>
...
const USER_ALLOWED_CODES=['CAA','CA','HPA','HP','RTT','CET','PR','DA','HS','CF','RPS'];
...
const ADMIN_CONGE_CODES=['CAA','CA','HPA','HP','RTT','CET','PR','DA','HS','CF','RPS','__EMPTY__','ASA','CAM','CONV','GEM','VEM','STG','CMO','CLM','CLD','__EMPTY__','__EMPTY__','__EMPTY__'];
...
(async function startPolling(){
  ...
  setInterval(async()=>{
    if(_modalOpen) return; // ne pas recharger pendant qu'un modal est ouvert
    const t = await fetchToken();
    if(t !== null && _pollToken !== null && t !== _pollToken){
      location.replace(location.pathname+'?annee='+ANNEE+'&mois='+MOIS+'&vue='+VUE);
    }
    if(t !== null) _pollToken = t;
  }, 5000);
})();
...
function openModal(td){
  ...
  const isFerieOrWE=(cycleVal==='FERIE'||cycleVal==='RC'||cycleVal==='RL');
  // Mode perm si : perm existante, OU cellule FERIE/WE ET agent habilité
  // LCL PARENT et ADC LAMBERT (lcl_parent/adc_lambert) ouvrent directement en modal congé
...
function openCongeModal(td,date,groupe,cycle,isPerm){
  ...
  if(isPerm){
    ...
    // Tous les boutons perm visibles (droits vérifiés côté PHP)
...
function buildGrid(g){
  ...
  if(GROUPES_SPECIAUX.includes(g)){
    ...
    const nbCodes=(TYPES_GROUPE[g]||[]).length;
    ...
    // Non-admin, groupe standard/police : filtrer + réordonner selon USER_ALLOWED_CODES
...
function buildVacOverrideSection(groupe, currentVac, vacId){
  if(!GROUPES_VAC_OVERRIDE.includes(groupe)){
    secVacOverride.style.display='none'; return;
  }
...
function buildPermTableau(data, mo){
  ...
  Object.entries(sections).forEach(([lbl,agents])=>{
    html+=`<tr style="background:#1a2742">
      <td colspan="8" style="padding:4px 10px;font-size:10.5px;font-weight:700;color:#ffd600;letter-spacing:.05em">${lbl}</td>
    </tr>`;
    ...
    agents.forEach((ag,i)=>{
      ...
      // Cellule spéciale Cne MOKADEM (0 partout = grisé)
      const allZero=t.total===0;

      html+=`<tr style="background:${allZero?'#f0f0f0':rowBg}">
        <td style="padding:5px 10px;font-weight:700;color:${allZero?'#999':'#1a2742'}">${ag}</td>
  ...
  if(mo===0){
    ...
    html+=`<h4 style="font-size:.82rem;font-weight:700;color:#1a5276;margin:18px 0 8px;padding:0 2px">&#128337; Permanences passées (${today.getFullYear()})</h4>`;
    ...
    tousAgents2.forEach((ag,i)=>{
      ...
      const tot=p+f;
      const rowBg=i%2?'#f0f4ff':'#fff';
      html+=`<tr style="background:${tot===0?'#f5f5f5':rowBg}">
        <td style="padding:4px 10px;font-weight:600;color:${tot===0?'#aaa':'#1a2742'}">${ag}</td>
  ...
  if(mo===0){
    html+=`<h4 style="font-size:.82rem;font-weight:700;color:#1565c0;margin:18px 0 8px;padding:0 2px">Récapitulatif mensuel</h4>`;
    html+='<div style="overflow-x:auto"><table style="border-collapse:collapse;font-size:11px;width:100%">';
    ...
    tousAgents.forEach(ag=>html+=`<th style="padding:5px 8px;text-align:center;white-space:nowrap">${ag.split(' ').pop()}</th>`);
    ...
    for(let m=1;m<=12;m++){
      ...
      tousAgents.forEach(ag=>{
        ...
        moisTot+=v;
        html+=`<td style="text-align:center;padding:4px 8px;${v>0?'font-weight:700;color:#1565c0':''}">${v>0?v:''}</td>`;
      });
...
document.getElementById('btn-open-perm').addEventListener('click',()=>{
  overlayPerm.classList.add('open');
  // Mettre Annuel actif visuellement
...
document.getElementById('btn-open-tir').addEventListener('click',()=>{
...
function buildTirAnnuel(r){
  ...
  const qStyle=[
    null,
    {bg:'#1565c0',col:'#ffd600',label:'Q1 (Jan – Avr)'},
    {bg:'#27ae60',col:'#fff',   label:'Q2 (Mai – Août)'},
    {bg:'#e65100',col:'#fff',   label:'Q3 (Sep – Déc)'},
  ];
  // Couleurs alternées par mois dans chaque Q (comme capture : alternance cyan/magenta/jaune...)
  const moisBg=[
    null,
    '#cce5ff','#b3d9ff','#99ccff','#80c0ff',   // Q1 : bleus dégradés
    '#c8f5c8','#aaf0aa','#8ce98c','#70e070',   // Q2 : verts dégradés
    '#ffe0b2','#ffcc80','#ffb74d','#ffa726',   // Q3 : oranges dégradés
  ];
  ...
  html+=`<tr style="text-align:center;font-weight:800">
    <th style="padding:5px 10px;background:#1a2742;color:#ffd600;text-align:left" rowspan="2">Agent</th>
    <th colspan="4" style="padding:5px 8px;background:${qStyle[1].bg};color:${qStyle[1].col};border-right:2px solid #fff">${qStyle[1].label}</th>
    <th colspan="4" style="padding:5px 8px;background:${qStyle[2].bg};color:${qStyle[2].col};border-right:2px solid #fff">${qStyle[2].label}</th>
    <th colspan="4" style="padding:5px 8px;background:${qStyle[3].bg};color:${qStyle[3].col};border-right:2px solid #fff">${qStyle[3].label}</th>
  ...
  for(let m=1;m<=12;m++){
    const sep=m===4||m===8?'border-right:2px solid #fff;':'';
    html+=`<th style="padding:4px 6px;background:${moisBg[m]};color:#1a2742;font-weight:700;${sep}min-width:22px">${moisInit[m-1]}</th>`;
  }
  html+='</tr></thead><tbody>';
  ...
  Object.entries(sectionsT).forEach(([lbl,agents])=>{
    const agsFilt=agents.filter(a=>tirSet.has(a));
    if(agsFilt.length===0) return;

    html+=`<tr style="background:#253560">
      <td colspan="15" style="padding:3px 10px;font-size:10px;font-weight:700;color:#ffd600;letter-spacing:.05em">${lbl}</td>
    </tr>`;
    ...
    agsFilt.forEach((ag,i)=>{
      ...
      html+=`<td style="padding:4px 10px;font-weight:700;white-space:nowrap;color:#1a2742">${ag}</td>`;

      for(let m=1;m<=12;m++){
        const tirs=(parMois[ag]&&parMois[ag][m])||[];
        const v=tirs.length;
        tot+=v;
        const sep=m===4||m===8?'border-right:2px solid #c5d8f0;':'';
        const tip=v>0?tirs.map(t=>t.date.split('-')[2]+'/'+t.date.split('-')[1]+' '+perLibTir[t.periode]).join(', '):'';
        // Cellule colorée si TIR effectué
        const cellBg=v>0?(m<=4?'#1565c0':m<=8?'#27ae60':'#e65100'):'transparent';
        const cellCol=v>0?'#fff':'#bbb';
        html+=`<td style="text-align:center;padding:4px 5px;background:${cellBg};color:${cellCol};font-weight:${v>0?'700':'400'};${sep}" title="${tip}">${v>0?v:'0'}</td>`;
      }
  ...
  totParMois.forEach((v,idx)=>{
    const m=idx+1;
    const sep=m===4||m===8?'border-right:2px solid #555;':'';
    html+=`<td style="text-align:center;padding:5px 5px;${sep};color:${v>0?'#ffd600':'#555'}">${v||'—'}</td>`;
  });
  ...
  html+=`<h4 style="font-size:.82rem;font-weight:700;color:#1565c0;margin:14px 0 6px;padding:0 2px">
...
const QUOTA_TYPES_J={CA:'Congé annuel',HP:'Heure prép.',HPA:'H.Prép. AM',RTT:'RTT',CET:'CET',CF:'Formation'};
// Types en heures
    ...
    Object.entries(types).forEach(([tc,q])=>{
      ...
      html+=`<div style="display:flex;align-items:center;gap:5px;margin-bottom:4px">
        ...
        <input type="number" min="0" max="9999" step="${isH?'0.01':'1'}" value="${q.quota}"
          ...
          style="width:52px;padding:2px 5px;border:1.5px solid ${over?'#c0392b':'#ccc'};border-radius:4px;font-size:.78rem;font-weight:700">
  ...
  btn.addEventListener('click',async()=>{
    ...
    if(r.ok){
      ...
      if(locked){
        ...
        row.querySelector('span:last-of-type').textContent='🔓 Libre';
        ...
        row.querySelector('span:last-of-type').textContent='🔒 Verrouillé';
...
async function loadUsers(){
  ...
  r.users.forEach(u=>{
    ...
    div.innerHTML=`<span class="user-badge badge-${u.role}">${u.role.toUpperCase()}</span>
      ...
      <button class="btn-action btn-perm btn-edit-user" data-u='${JSON.stringify(u)}' style="padding:2px 8px;font-size:.72rem">✏️</button>
...
document.getElementById('btn-save-user').addEventListener('click',async()=>{
  ...
  if(r.ok){showToast('Utilisateur enregistré');document.getElementById('user-form-wrap').style.display='none';loadUsers();}
  else showToast(r.msg||'Erreur',false);
});
