...
<div class="overlay-perm" id="overlay-recap-fetes">
  <div style="background:#fff;border-radius:14px;box-shadow:0 8px 40px rgba(0,0,0,.35);
    ...
    <div style="background:#7b1fa2;color:#fff;padding:12px 18px;display:flex;align-items:center;justify-content:space-between;flex-shrink:0">
      <span style="font-size:.9rem;font-weight:800;letter-spacing:.04em">🎄 Récapitulatif permanences fêtes — 25/12 · 31/12 · 01/01</span>
...
const ADMIN_CONGE_CODES=['CAA','CA','HPA','HP','RTT','CET','PR','DA','HS','CF','RPS','__EMPTY__','ASA','CAM','CONV','GEM','VEM','STG','CMO','CLM','CLD','AUT','PREV','__EMPTY__'];
...
function openModal(td){
  ...
  const groupeLibre=['gie','gie_j','lcl_parent','adc_lambert'];
...
document.getElementById('btn-open-recap-fetes').addEventListener('click',()=>{
...
// Format : { annee: { '25/12': [agents...], '31/12': [agents...], '01/01': [agents...] } }
...
const RECAP_FETES_DATA = {
  ...
  2020: {
    '25/12': {'BC BOUXOM':'AM','BC HOCHARD':'M'},
    '31/12': {'BC BASTIEN':'AM','BC HOCHARD':'M'},
    '01/01': {'ADJ LEFEBVRE':'M'},
  },
  2021: {
    '25/12': {'ADJ CORRARD':'M','ADJ LEFEBVRE':'AM','GP DHALLEWYN':'M'},
    '31/12': {'BC BASTIEN':'M','ADJ LEFEBVRE':'AM'},
    '01/01': {'BC HOCHARD':'AM','ADJ CORRARD':'M'},
  },
  2022: {
    '25/12': {'BC BOUXOM':'AM','GP DHALLEWYN':'AM'},
    '31/12': {'ADJ CORRARD':'M','BC DUPUIS':'AM'},
    '01/01': {'BC BASTIEN':'M','BC ANTHONY':'AM'},
  },
  2023: {
    '25/12': {'BC ARNAULT':'AM','BC BASTIEN':'M','ADJ CORRARD':'M'},
    '31/12': {'ADJ LEFEBVRE':'M','BC HOCHARD':'AM'},
    '01/01': {'BC BOUXOM':'M','GP DHALLEWYN':'AM'},
  },
  2024: {
    '25/12': {'BC DUPUIS':'AM','BC ANTHONY':'M','BC DELCROIX':'AM'},
    '31/12': {'BC BOUXOM':'M','BC ARNAULT':'AM'},
    '01/01': {'ADJ LEFEBVRE':'M','ADJ CORRARD':'AM','BC DRUEZ':'M'},
  },
  2025: {
    '25/12': {'BC BASTIEN':'AM','BC HOCHARD':'M'},
    '31/12': {'BC DUPUIS':'AM','BC ANTHONY':'M'},
    '01/01': {'BC ARNAULT':'M','GP DHALLEWYN':'AM'},
  },
};

const FETES_JOURS = ['25/12','31/12','01/01'];
const FETES_LABELS = {'25/12':'🎄 25 Déc','31/12':'🥂 31 Déc','01/01':'🎆 1er Jan'};
...
document.querySelectorAll('.btn-toggle-lock-mois').forEach(btn => {
  btn.addEventListener('click', async () => {
    ...
    if (r.ok) {
      ...
      // Mettre à jour les cellules du tableau planning
...
document.addEventListener('click', async e => {
  ...
  if (r.ok) {
    if (locked) {
      ...
      // Mettre à jour LOCKED_AGENTS_MOIS en mémoire
      if (LOCKED_AGENTS_MOIS[ag]) delete LOCKED_AGENTS_MOIS[ag][mois];
      // Retirer locked-agent sur les cellules du mois
      document.querySelectorAll('td[data-agent="'+ag+'"]').forEach(cell => {
      ...
      if (!LOCKED_AGENTS_MOIS[ag]) LOCKED_AGENTS_MOIS[ag] = {};
      LOCKED_AGENTS_MOIS[ag][mois] = true;
      // Ajouter locked-agent sur toutes les cellules de l'agent
      document.querySelectorAll('td[data-agent="'+ag+'"]').forEach(cell => {
...
document.getElementById('btn-open-quota-recap')?.addEventListener('click', async ()=>{
  ...
  const TYPE_LABELS = {CA:'Congé annuel',HP:'Congés Annuels Hors Période',HPA:'Congés Annuels Antérieurs Hors Période', CAM: 'Congés Annuels Maladie',RTT:'RTT',CET:'CET',CF:'Formation (j)',RTC:'RTC (h)',CF_H:'CF (h)'};
  let html='';
...
(function(){
  ...
  window.loadQuotaWidget = async function() {
    ...
    const TYPE_LBL = {CA:'Congé annuel',HP:'Congés Annuels Hors Période',HPA:'Congés Annuels Antérieur Hors Période', CAM: 'Congés Annuels Maladie',RTT:'RTT',CET:'CET',CF:'Formation'};
    let html = '';
