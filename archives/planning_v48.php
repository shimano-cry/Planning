...
const CR_GROUPES = [
  ...
  { lbl:'Douane',        agents:['IR MOREAU','ACP1 DEMERVAL','ACP1 LOIN'] },
...
async function loadCongesRecap(){
  ...
  Object.values(quotas).forEach(q=>Object.keys(q).forEach(t=>{ if((quotas[Object.keys(quotas)[0]]||{})[t]?.unite!=='heures') typesSet.add(t); }));
  // Types présents dans les compteurs mais pas dans les quotas (ex: Douane)
  Object.values(counts).forEach(c=>Object.keys(c).forEach(t=>typesSet.add(t)));
  // Retirer les types "heures" des quotas connus
  Object.values(quotas).forEach(q=>Object.entries(q).forEach(([t,v])=>{ if(v.unite==='heures') typesSet.delete(t); }));
  const TYPES = [...typesSet].filter(t=>!['RTC'].includes(t));
  ...
  TYPES.forEach(t=>{
    const tot = Object.values(counts).reduce((s,c)=>s+(c[t]||0),0);
