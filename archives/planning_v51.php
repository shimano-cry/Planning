  ...
  <div class="modal-perm" id="modal-tir">
    <div class="mp-head" style="background:#1565c0">
      ...
      <div style="display:flex;align-items:center;gap:8px">
        <button id="btn-expand-tir" title="Agrandir / Réduire" style="background:rgba(255,255,255,.18);border:none;color:#fff;font-size:1.1rem;cursor:pointer;border-radius:5px;padding:2px 8px;line-height:1.3;transition:background .15s" onmouseenter="this.style.background='rgba(255,255,255,.32)'" onmouseleave="this.style.background='rgba(255,255,255,.18)'">&#x26F6;</button>
...
// ── Agrandir / Réduire la fenêtre TIR ──────────────────────────────────────
const modalTirEl = document.getElementById('modal-tir');
let tirExpanded = false;
const btnExpandTir = document.getElementById('btn-expand-tir');
if(btnExpandTir){
  btnExpandTir.addEventListener('click', ()=>{
    tirExpanded = !tirExpanded;
    if(tirExpanded){
      modalTirEl.style.width  = '98vw';
      modalTirEl.style.maxWidth = '98vw';
      modalTirEl.style.maxHeight = '96vh';
      overlayTir.style.paddingTop = '10px';
      btnExpandTir.innerHTML = '&#x2922;'; // réduire
      btnExpandTir.title = 'Réduire';
    } else {
      modalTirEl.style.width  = '';
      modalTirEl.style.maxWidth = '';
      modalTirEl.style.maxHeight = '';
      overlayTir.style.paddingTop = '';
      btnExpandTir.innerHTML = '&#x26F6;'; // agrandir
      btnExpandTir.title = 'Agrandir / Réduire';
    }
  });
}
...
function buildTirAnnuel(r){
  ...
  // ── Ligne total global par quadrimestre (agents ayant effectué / total agents) ──
  if(IS_ADMIN){
    const visAgents = tirAgents; // admin : tous les agents
    const nbTotal = visAgents.length;
    html+=`<tr style="background:#1a2742;color:#ffd600;font-weight:800">`;
    html+=`<td style="padding:5px 10px;color:#ffd600;font-size:.78rem;white-space:nowrap">TOTAL global</td>`;
    Q_DEF.forEach(q=>{
      const nbOk = visAgents.filter(ag=>{ const aq=quadri[ag]||{}; const qd=aq[q.num]; return qd&&qd.nb>=1; }).length;
      const ratio = `${nbOk}/${nbTotal}`;
      const col = nbOk===nbTotal?'#69f0ae':nbOk===0?'#ff6b6b':'#ffd600';
      const qBg = q.num===1?'#1565c0':q.num===2?'#2e7d32':'#bf360c';
      html+=`<td style="text-align:center;padding:5px 10px;background:${qBg};color:${col};font-size:.82rem;font-weight:800">${ratio}</td>`;
    });
    // Colonne Total / 3 : agents ayant effectué les 3 quadrimestres / total
    const nbFull = visAgents.filter(ag=>{ const aq=quadri[ag]||{}; return Q_DEF.every(q=>{ const qd=aq[q.num]; return qd&&qd.nb>=1; }); }).length;
    const fullCol = nbFull===nbTotal?'#69f0ae':nbFull===0?'#ff6b6b':'#ffd600';
    html+=`<td style="text-align:center;padding:5px 10px;background:#c0392b;color:${fullCol};font-size:.82rem;font-weight:800">${nbFull}/${nbTotal}</td>`;
    html+='</tr>';
  }
