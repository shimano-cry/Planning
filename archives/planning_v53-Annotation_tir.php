...
// Table tir_annotations : annotations d'exemption/dispense de TIR pour un agent/quadrimestre/année
$mysqli->query("CREATE TABLE IF NOT EXISTS tir_annotations (
    id         INT AUTO_INCREMENT PRIMARY KEY,
    agent      VARCHAR(80) NOT NULL,
    annee      INT NOT NULL,
    quadri     TINYINT NOT NULL COMMENT '1, 2 ou 3',
    motif      VARCHAR(200) NOT NULL DEFAULT '',
    created_by INT DEFAULT NULL,
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    UNIQUE KEY uk_agent_annee_quad (agent,annee,quadri)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_SERVER['HTTP_X_REQUESTED_WITH'])) {
    ...
    /* ── Changer son propre mot de passe ── */
        ...
        // Récupérer congés avec leur type
        $stC = $mysqli->prepare("SELECT agent,date_debut,date_fin,type_conge FROM conges WHERE YEAR(date_debut)=? OR YEAR(date_fin)=?");
        $stC->bind_param('ii',$yr,$yr);
        $stC->execute();
        $resC = $stC->get_result();
        $counts = []; // [agent][type] = nb jours ouvrés
            ...
            while ($d <= $fin) {
                ...
                if ($yr2 === $yr) {
                    ...
                    if ($dow < 6 && !isset($feriesAnnee[$ds])) {
                        ...
                        $counts[$ag][$type] = ($counts[$ag][$type]??0) + 1;
                        // Compter les CA posés en périodes basses (jan-avr et oct-déc)
                        if ($type === 'CA') {
                            $mois = (int)$d->format('n');
                            if ($mois <= 4 || $mois >= 10) {
                                $ca_hors_periode[$ag] = ($ca_hors_periode[$ag]??0) + 1;
                            }
                        }
                    }
                }
    ...
    /* ── Compteur TIR ── */
        ...
        // Annotations exemption/dispense par agent et quadrimestre
        $annotations=[];
        $stAnnot=$mysqli->prepare("SELECT agent,quadri,motif FROM tir_annotations WHERE annee=?");
        $stAnnot->bind_param('i',$yr);
        $stAnnot->execute();
        $res5=$stAnnot->get_result();
        while($r=$res5->fetch_assoc()){
            $annotations[$r['agent']][(int)$r['quadri']] = $r['motif'];
        }
        echo json_encode(['ok'=>true,'annuel'=>$annuel,'parMois'=>$parMois,'quadri'=>$quadri,
                          'mensuel'=>$mensuel,'detailQuad'=>$detailQuad,
                          'annulations'=>$annulations,'annotations'=>$annotations,
                          'annee'=>$yr,'mois'=>$mo,'quad'=>$qr]);

    } elseif ($action==='save_tir_annotation') {
        // Admin uniquement
        if (!$isAdmin) { echo json_encode(['ok'=>false,'msg'=>'Accès refusé']); exit; }
        $agent = trim($_POST['agent'] ?? '');
        $yr    = (int)($_POST['annee'] ?? $annee);
        $qr    = (int)($_POST['quadri'] ?? 0);
        $motif = trim($_POST['motif'] ?? '');
        if (!$agent || $qr<1 || $qr>3) { echo json_encode(['ok'=>false,'msg'=>'Paramètres invalides']); exit; }
        if ($motif === '') {
            // Suppression de l'annotation
            $st=$mysqli->prepare("DELETE FROM tir_annotations WHERE agent=? AND annee=? AND quadri=?");
            $st->bind_param('sii',$agent,$yr,$qr);
            $st->execute();
        } else {
            // Insertion ou mise à jour
            $st=$mysqli->prepare("INSERT INTO tir_annotations (agent,annee,quadri,motif,created_by) VALUES (?,?,?,?,?) ON DUPLICATE KEY UPDATE motif=VALUES(motif),updated_at=NOW()");
            $st->bind_param('siisi',$agent,$yr,$qr,$motif,$userId);
            $st->execute();
        }
        echo json_encode(['ok'=>true,'agent'=>$agent,'annee'=>$yr,'quadri'=>$qr,'motif'=>$motif]);
...
.modal-admin{background:#fff;border-radius:12px;width:100%;max-width:820px;max-height:90vh;display:flex;flex-direction:column;box-shadow:0 8px 32px rgba(0,0,0,.25);overflow:hidden}
...
<div class="overlay-perm" id="overlay-recap-fetes">
  <div style="background:#fff;border-radius:14px;box-shadow:0 8px 40px rgba(0,0,0,.35);
    ...
    <div style="background:#7b1fa2;color:#fff;padding:12px 18px;display:flex;align-items:center;justify-content:space-between;flex-shrink:0">
      <span style="font-size:.9rem;font-weight:800;letter-spacing:.04em">🎄 Récapitulatif permanences fêtes — 01/01 · 25/12 · 31/12</span>
...
function openModal(td){
  ...
  if(!IS_ADMIN && !groupeLibre.includes(td.dataset.groupe) && (curCongeId>0 || curPermId>0)){
...
function openCongeModal(td,date,groupe,cycle,isPerm){
  ...
  if(isPerm){
    ...
    // Admin : tous les boutons. Non-admin : indisponibilités seulement (M/AM masqués)
    if(IS_ADMIN){
      ...
      // Non-admin : uniquement les indisponibilités
    ...
    if(permGrid){
      if(IS_ADMIN){
        ...
        // Non-admin : 3 boutons indispo sur une ligne
  ...
  ensureQuotaCache(); // Précharger quota en arrière-plan
    ...
    if(!IS_ADMIN && !IS_DOUANE && !GRP_NO_PREV_BANNER.includes(groupe)){
      if(!prevBanner){
...
function buildTirAnnuel(r){
  ...
  html+=`<tr style="text-align:center;font-weight:800">
    <th style="padding:5px 10px;background:#1a2742;color:#ffd600;text-align:center;font-size:15px" rowspan="2">Agents</th>
  ...
  // Non-admin : restreindre l'affichage aux seuls agents de l'utilisateur connecté
  const userSet=IS_ADMIN ? null : new Set(USER_AGENTS);
  const annotations=r.annotations||{};
  let grandTot=0;

  Object.entries(sectionsT).forEach(([lbl,agents])=>{
    const agsFilt=agents.filter(a=>tirSet.has(a) && (!userSet || userSet.has(a)));
    ...
    agsFilt.forEach((ag,i)=>{
      ...
      // Annotation par quadrimestre pour cet agent
      const agAnnot=annotations[ag]||{};
      ...
      for(let m=1;m<=12;m++){
        ...
        const qNum=m<=4?1:m<=8?2:3;
        const hasAnnot=!!agAnnot[qNum];
        // Cellule : fond pâle quadrimestre, chiffre en couleur foncée ; si TIR fait → fond coloré + blanc
        // Si annotation exemption et pas de TIR → fond hachuré violet pâle
        let cellBg,cellCol,disp;
        if(v>0){
          cellBg=tirDoneBg[m]; cellCol='#fff'; disp=v;
        } else if(hasAnnot){
          cellBg='repeating-linear-gradient(135deg,#f3e5f5,#f3e5f5 4px,#ede7f6 4px,#ede7f6 8px)';
          cellCol='#7b1fa2'; disp='○';
        } else {
          cellBg=moisBg[m]; cellCol=moisCol[m]+'88'; disp='·';
        }
      ...
      // ── Ligne annotations exemption (si au moins une pour cet agent) ──
      const annotKeys=Object.keys(agAnnot).filter(q=>agAnnot[q]);
      if(annotKeys.length>0){
        const annotParts=annotKeys.sort().map(q=>{
          const qNum=parseInt(q);
          const qBg=[null,'#1565c0','#2e7d32','#bf360c'][qNum];
          const editAttr=IS_ADMIN?` style="cursor:pointer;text-decoration:underline dotted" onclick="tirAnnotEdit('${ag}',${qNum},'${(agAnnot[q]||'').replace(/'/g,"\\'")}')"`:'';
          return `<span style="display:inline-flex;align-items:center;gap:3px;background:${qBg};color:#fff;padding:1px 7px;border-radius:10px;font-size:.68rem;font-weight:700">Q${qNum}</span>`+
            `<span${editAttr} style="font-size:.68rem;color:#7b1fa2;font-style:italic">📝 ${agAnnot[q]}</span>`;
        });
        html+=`<tr style="background:${rowBg}">
          <td colspan="15" style="padding:2px 10px 5px 10px;border-top:none">
            <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">
              <span style="font-size:.68rem;color:#7b1fa2;font-weight:600">Annotation :</span>
              ${annotParts.join('')}
              ${IS_ADMIN?`<span style="font-size:.65rem;color:#bbb;cursor:pointer" onclick="tirAnnotEdit('${ag}',0,'')">+ ajouter</span>`:''}
            </div>
          </td>
        </tr>`;
      } else if(IS_ADMIN){
        // Pour l'admin, lien discret pour ajouter une annotation
        html+=`<tr style="background:${rowBg}">
          <td colspan="15" style="padding:1px 10px 3px 10px;border-top:none">
            <span style="font-size:.63rem;color:#ccc;cursor:pointer" onclick="tirAnnotEdit('${ag}',0,'')" title="Ajouter une annotation d'exemption/dispense">+ annotation</span>
          </td>
        </tr>`;
      }
  ...
  // ── Ligne TOTAL global : agents ayant effectué au moins 1 TIR / quadrimestre ──
  const nbTotalAg = tirAgents.length;
  html+=`<tr style="background:#253560;color:#ffd600;font-weight:800;font-size:13px">`;
  html+=`<td style="padding:5px 10px;color:#aac8ff;font-size:.75rem;white-space:nowrap">TOTAL global</td>`;
  // Une cellule par quadrimestre (colspan=4), indiquant le nb d'agents ayant fait ≥1 TIR dans le quadrimestre
  Q_DEF.forEach(q=>{
    const qBg=[null,'#1565c0','#2e7d32','#bf360c'][q.num];
    const nbOkQuad=tirAgents.filter(ag=>q.mois.some(m=>(parMois[ag]&&parMois[ag][m]&&parMois[ag][m].length>0))).length;
    const col=nbOkQuad===nbTotalAg?'#69f0ae':nbOkQuad===0?'#aaa':'#ffd600';
    const sepL=q.num===1?'border-left:3px solid #555;':'';
    const sepR='border-right:4px solid #555;';
    html+=`<td colspan="4" style="text-align:center;padding:5px 5px;${sepL}${sepR};background:${qBg}cc;color:${col};font-size:.85rem;font-weight:800">${nbOkQuad>0?nbOkQuad+'/'+nbTotalAg:'—'}</td>`;
  });
  // Colonne TOTAL : agents ayant fait ≥1 TIR dans l'année
  ...
  html+=`<div id="${quadWrapId}">`;
  html+='<div style="font-size:.7rem;color:#555;margin-bottom:8px">'
    +'✅ Effectué &nbsp;|&nbsp; ❌ Manquant &nbsp;|&nbsp; '
    +'<span style="background:#fff8e1;padding:1px 5px;border-radius:3px;color:#e65100">⏳ En cours</span>'
    +' &nbsp;|&nbsp; <span style="background:#f3e5f5;padding:1px 5px;border-radius:3px;color:#7b1fa2">📝 Annoté (exempté/dispensé)</span></div>';
  ...
  tirAgents.filter(ag=>IS_ADMIN || USER_AGENTS.includes(ag)).forEach((ag,i)=>{
    const aq=quadri[ag]||{};
    const agAnnot=annotations[ag]||{};
    ...
    Q_DEF.forEach(q=>{
      ...
      }).join(', '):(motif?'📝 '+motif:'Aucun TIR');
      let bg,col,icon;
      if(nb>=1){bg='#e8f5e9';col='#1b5e20';icon='✅';}
      else if(motif){bg='#f3e5f5';col='#7b1fa2';icon='📝';}
      ...
      const editCb=IS_ADMIN?` onclick="tirAnnotEdit('${ag}',${q.num},'${motif.replace(/'/g,"\\'")}')"`:'';
      html+=`<td style="text-align:center;padding:4px 8px;background:${bg};color:${col};font-weight:700;cursor:${IS_ADMIN?'pointer':'default'}" title="${tip}"${editCb}>${icon}${plus}</td>`;
    });
...
/* ── Annotation TIR : dialog admin pour saisir exemption/dispense ── */
function tirAnnotEdit(agent,quadriDefault,motifActuel){
  // Dialog simple pour choisir le quadrimestre et saisir le motif
  ...
  const Q_LABELS=['','Q1 (Jan–Avr)','Q2 (Mai–Août)','Q3 (Sep–Déc)'];
  const qSel=quadriDefault>0?quadriDefault:1;
  dlg.innerHTML=`
    <div style="background:#fff;border-radius:10px;padding:22px 26px;min-width:340px;max-width:95vw;box-shadow:0 8px 40px rgba(0,0,0,.3)">
      <h4 style="margin:0 0 14px;color:#1a2742;font-size:1rem">📝 Annotation TIR — <span style="color:#7b1fa2">${agent}</span></h4>
      <label style="font-size:.8rem;color:#555;display:block;margin-bottom:4px">Quadrimestre concerné</label>
      <select id="tir-annot-quad" style="width:100%;padding:7px;border:1px solid #ccc;border-radius:5px;margin-bottom:12px;font-size:.9rem">
        ${[1,2,3].map(q=>`<option value="${q}"${q===qSel?' selected':''}>${Q_LABELS[q]}</option>`).join('')}
      </select>
      <label style="font-size:.8rem;color:#555;display:block;margin-bottom:4px">Motif (vide = supprimer l'annotation)</label>
      <input id="tir-annot-motif" type="text" value="${motifActuel.replace(/"/g,'&quot;')}"
        placeholder="Ex : Dispensé – maternité, Exempté – blessure…"
        style="width:100%;box-sizing:border-box;padding:7px;border:1px solid #ccc;border-radius:5px;font-size:.9rem;margin-bottom:16px">
      <div style="display:flex;gap:10px;justify-content:flex-end">
        <button onclick="this.closest('div[style*=fixed]').remove()"
          style="padding:7px 16px;border:1px solid #ccc;border-radius:5px;background:#f5f5f5;cursor:pointer;font-size:.85rem">Annuler</button>
        <button id="tir-annot-save"
          style="padding:7px 16px;border:none;border-radius:5px;background:#7b1fa2;color:#fff;cursor:pointer;font-size:.85rem;font-weight:700">Enregistrer</button>
      </div>
      <p id="tir-annot-msg" style="margin:8px 0 0;font-size:.78rem;text-align:center;color:#888"></p>
    </div>`;
  document.body.appendChild(dlg);
  dlg.querySelector('#tir-annot-motif').focus();
  dlg.querySelector('#tir-annot-save').addEventListener('click',async()=>{
    const qv=parseInt(dlg.querySelector('#tir-annot-quad').value);
    const mv=dlg.querySelector('#tir-annot-motif').value.trim();
    const msg=dlg.querySelector('#tir-annot-msg');
    msg.textContent='Enregistrement…'; msg.style.color='#888';
    const res=await ajax({action:'save_tir_annotation',agent,annee:ANNEE,quadri:qv,motif:mv});
    if(res&&res.ok){
      msg.textContent='✅ Enregistré'; msg.style.color='#27ae60';
      setTimeout(()=>{dlg.remove(); loadTir();},700);
    } else {
      msg.textContent='❌ Erreur'; msg.style.color='#c0392b';
    }
  });
  // Fermer si clic hors dialog
...
// Format : { annee: { '01/01': [agents...], '25/12': [agents...], '31/12': [agents...] } }
...
const RECAP_FETES_DATA = {
  ...
  2020: {
    '01/01': {'ADJ LEFEBVRE':'M'},
    '25/12': {'BC BOUXOM':'AM','BC HOCHARD':'M','BC BASTIEN':'AM'},
    '31/12': {'BC BASTIEN':'M','BC BOUXOM':'M'},
  },
  2021: {
    '01/01': {'BC DELCROIX':'M','BC DRUEZ':'AM'},
    '25/12': {'GP DHALLEWYN':'M'},
    '31/12': {'ADJ LEFEBVRE':'M','ADJ CORRARD':'AM','BC BASTIEN':'M','BC HOCHARD':'AM'},
  },
  2022: {
    '01/01': {'ADJ LEFEBVRE':'M','BC DRUEZ':'AM'},
    '25/12': {'BC BOUXOM':'AM','BC ARNAULT':'AM'},
    '31/12': {'ADJ CORRARD':'AM'},
  },
  2023: {
    '01/01': {'ADJ CORRARD':'AM'},
    '25/12': {'GP DHALLEWYN':'AM','BC DUPUIS':'M'},
    '31/12': {'BC BASTIEN':'M','BC DRUEZ':'AM'},
  },
  2024: {
    '01/01': {'BC BASTIEN':'AM','BC DRUEZ':'AM'},
    '25/12': {'BC ARNAULT':'AM','BC BOUXOM':'M'},
    '31/12': {'ADJ LEFEBVRE':'M','ADJ CORRARD':'AM'},
  },
  2025: {
    '01/01': {'ADJ LEFEBVRE':'M','BC DELCROIX':'AM'},
    '25/12': {'BC HOCHARD':'AM','ADJ CORRARD':'M'},
    '31/12': {'BC DUPUIS':'AM','BC ANTHONY':'AM','BC BASTIEN':'AM','BC BOUXOM':'M','BC ARNAULT':'M'},
  },
};

const FETES_JOURS = ['01/01','25/12','31/12'];
const FETES_LABELS = {'01/01':'🎆 1er Jan','25/12':'🎄 25 Déc','31/12':'🥂 31 Déc'};
const FETES_COLORS_M  = {'25/12':'#1565c0','31/12':'#7b1fa2','01/01':'#c0392b'};
const FETES_COLORS_AM = {'25/12':'#42a5f5','31/12':'#ab47bc','01/01':'#ef5350'};
const FETES_BG        = {'25/12':'#e3f0ff','31/12':'#f3e5f5','01/01':'#fdecea'};
...
function buildRecapFetes(){
  ...
  annees.forEach(an=>{
    html+=`<th colspan="3" style="padding:5px 8px;background:#4a0072;color:#fff;text-align:center;border-left:3px solid #7b1fa2;min-width:114px">${an}</th>`;
  });
  html+='</tr>';
  ...
  RECAP_AGENTS.forEach((ag,ai)=>{
    const rowBg=ai%2===0?'#f9f9fb':'#fff';
    html+=`<tr style="background:${rowBg}">`;
    html+=`<td style="padding:3px 10px;font-weight:600;white-space:nowrap;background:${rowBg};position:sticky;left:0;border-right:2px solid #dde">${ag}</td>`;
...
async function loadCongesRecap(){
  ...
  <thead>
    <tr style="background:#1a2742;color:#ffd600;text-align:center">
      <th style="text-align:left;padding:6px 10px;min-width:130px;position:sticky;left:0;background:#1a2742">Agent</th>`;
  TYPES.forEach(t=>{ html+=`<th style="padding:5px 7px;white-space:nowrap" colspan="2">${CR_TYPE_LBL[t]||t}</th>`; });
  html+=`</tr>
    <tr style="background:#253560;color:#aac8ff;text-align:center;font-size:10px">
      <th style="position:sticky;left:0;background:#253560"></th>`;
  TYPES.forEach(()=>{ html+=`<th style="padding:3px 5px">Posé</th><th style="padding:3px 5px">Reste</th>`; });
  html+=`</tr></thead><tbody>`;
  ...
  CR_GROUPES.forEach(({lbl, agents})=>{
    ...
    agents.forEach((ag,i)=>{
      ...
      html+=`<tr style="background:${rowBg}">
        <td style="padding:5px 10px;font-weight:700;color:#1a2742;position:sticky;left:0;background:${rowBg};white-space:nowrap">${ag}</td>`;
      TYPES.forEach(t=>{
        ...
        html+=`<td style="text-align:center;padding:4px 6px;color:${posCol};font-weight:${used>0?'700':'400'}">${used||'—'}</td>`;
        html+=`<td style="text-align:center;padding:4px 6px;font-weight:${hasQ?'700':'400'};color:${hasQ?restCol:'#ccc'}">${hasQ?(over?'⚠'+reste:reste):'—'}</td>`;
      });
      html+='</tr>';
    });
  });

  // Ligne totaux par type (hors agents douane exclus)
  html+=`<tr style="background:#1a2742;color:#ffd600;font-weight:800">
    <td style="padding:6px 10px;position:sticky;left:0;background:#1a2742">TOTAL posé</td>`;
  TYPES.forEach(t=>{
    const tot = Object.entries(counts).filter(([ag])=>!CR_DOUANE_EXCLUS.includes(ag)).reduce((s,[,c])=>s+(c[t]||0),0);
    html+=`<td style="text-align:center;padding:5px 6px;color:#ffd600">${tot||'—'}</td><td></td>`;
  });
