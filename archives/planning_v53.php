...
function buildTirAnnuel(r){
  ...
  // Déterminer la section de l'agent connecté pour le highlight
  ...
  for(let m=1;m<=12;m++){
    ...
    const qMois=Q_DEF[qNum-1].mois;
    // Nb agents ayant fait ≥1 TIR ce mois précis
    const nbOkMois=tirAgents.filter(ag=>(parMois[ag]&&parMois[ag][m]&&parMois[ag][m].length>0)).length;
    const col=nbOkMois===nbTotalAg?'#69f0ae':nbOkMois===0?'#555':'#ffd600';
    html+=`<td style="text-align:center;padding:5px 5px;${sepL}${sepR};color:${col};font-size:.72rem">${nbOkMois>0?nbOkMois+'/'+nbTotalAg:'—'}</td>`;
  }
