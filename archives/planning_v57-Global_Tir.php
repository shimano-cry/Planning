...
function buildTirAnnuel(r){
  ...
  Object.entries(sectionsT).forEach(([lbl,agents])=>{
    ...
    html+=`<tr style="background:${secBg}">
      <td colspan="15" style="padding:3px 10px;font-size:10px;font-weight:700;color:${secCol};letter-spacing:.05em">${lbl}${isMySection?'<span style="font-size:9px;margin-left:6px;background:#ffd600;color:#1a2742;padding:1px 5px;border-radius:3px">MON ÉQUIPE</span>':''}</td>
    </tr>`;
  ...
  // ── Lignes totaux admin (4 lignes : Q1 / Q2 / Q3 / Année) ──
  if(IS_ADMIN){
  const nbTotalAg=tirAgents.length;
  // Couleurs des quadrimestres (cohérent avec les en-têtes)
  const qMeta=[
    {num:1,label:'Q1 — Jan/Avr',mois:[1,2,3,4],bg:'#0d3b6e',border:'#1565c0'},
    {num:2,label:'Q2 — Mai/Août',mois:[5,6,7,8],bg:'#1b4a2a',border:'#2e7d32'},
    {num:3,label:'Q3 — Sep/Déc',mois:[9,10,11,12],bg:'#6d2000',border:'#bf360c'},
  ];

  qMeta.forEach(q=>{
    // Nb agents ayant fait ≥1 TIR sur l'ensemble du quadrimestre
    const nbOkQ=tirAgents.filter(ag=>q.mois.some(m=>parMois[ag]&&parMois[ag][m]&&parMois[ag][m].length>0)).length;
    const col=nbOkQ===nbTotalAg?'#69f0ae':nbOkQ===0?'#555':'#ffd600';
    html+=`<tr style="background:${q.bg};font-weight:800;font-size:11.5px">`;
    html+=`<td style="padding:4px 10px;color:#ffd600;white-space:nowrap;border-left:3px solid ${q.border}">${q.label}</td>`;
    for(let m=1;m<=12;m++){
      const inQ=q.mois.includes(m);
      const isFirst=m===1;
      const isLast=m===4||m===8||m===12;
      const sepR=isLast?'border-right:4px solid #555;':'';
      const sepL=isFirst?'border-left:3px solid #555;':'';
      if(!inQ){
        // Mois hors quadrimestre : cellule grisée
        html+=`<td style="background:#111927;${sepL}${sepR}"></td>`;
      } else {
        const nbOkM=tirAgents.filter(ag=>(parMois[ag]&&parMois[ag][m]&&parMois[ag][m].length>0)).length;
        const cM=nbOkM===nbTotalAg?'#69f0ae':nbOkM===0?'#555':'#ffd600';
        html+=`<td style="text-align:center;padding:4px 5px;${sepL}${sepR};color:${cM};font-size:.7rem">${nbOkM>0?nbOkM+'/'+nbTotalAg:'—'}</td>`;
      }
    }
    // Colonne TOTAL : X/N agents pour ce quadrimestre
    html+=`<td style="text-align:center;padding:4px 8px;font-weight:800;font-size:13px;color:${col}">${nbOkQ}/${nbTotalAg}</td>`;
    html+=`<td style="background:#4a0072"></td>`;
    html+='</tr>';
  });

  // Ligne année complète
  ...
  const gtot=tirAgents.reduce((s,ag)=>s+Object.values(parMois[ag]||{}).reduce((ss,arr)=>ss+arr.length,0),0);
  html+=`<tr style="background:#1a2742;color:#fff;font-weight:800;font-size:12px">`;
  html+=`<td style="padding:5px 10px;color:#ffd600;font-size:13px;white-space:nowrap">Année ${ANNEE}</td>`;
  for(let m=1;m<=12;m++){
    const isFirst=m===1;
    const isLast=m===4||m===8||m===12;
    const sepR=isLast?'border-right:4px solid #555;':'';
    const sepL=isFirst?'border-left:3px solid #555;':'';
    const v=tirAgents.reduce((s,ag)=>s+((parMois[ag]&&parMois[ag][m])?parMois[ag][m].length:0),0);
    const tcol=v>0?'#ffd600':'#555';
    html+=`<td style="text-align:center;padding:5px 5px;${sepL}${sepR};color:${tcol};font-size:.7rem">${v||'—'}</td>`;
  }
  html+=`<td style="text-align:center;padding:5px 8px;font-weight:800;font-size:14px;color:${totAnneeCol}">${nbOkAnnee}/${nbTotalAg}</td>`;
  html+=`<td style="background:#4a0072"></td>`;
  html+='</tr>';
