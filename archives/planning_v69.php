...
.login-center-row{
  ...
  gap:40px;width:100%;justify-content:center;
}
...
<div class="login-wrap">
  ...
  <div class="login-center-row">
    ...
    <!-- Logo gauche -->
    <img class="login-side-logo" src="Logo_CCPD.png" alt="Logo CCPD" onerror="this.style.display='none'">
...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_SERVER['HTTP_X_REQUESTED_WITH'])) {
    ...
    if ($action==='save_conge') {
        ...
        // ── Blocage PREV si congé admin déjà posé sur ce jour ──────────────────
        // Règle : un utilisateur (non-admin) ne peut pas poser PREV sur un jour
        // où l'admin a déjà enregistré un congé (ex : VEM, CA, etc.)
        if (!$isAdmin && $type === 'PREV' && $conge_id === 0) {
            $stCheck = $mysqli->prepare(
                "SELECT id, type_conge FROM conges
                  WHERE agent = ?
                    AND date_debut <= ?
                    AND date_fin   >= ?
                    AND type_conge <> 'PREV'
            ...
            $stCheck->bind_param('sss', $agent, $date_debut, $date_debut);
            $stCheck->execute();
            ...
            if ($rCheck && $rCheck->num_rows > 0) {
                ...
                echo json_encode([
                    'ok'  => false,
                    'msg' => '⛔ Un congé (' . htmlspecialchars($rowCheck['type_conge'], ENT_QUOTES, 'UTF-8') . ') a déjà été posé sur cette date par l\'administrateur. Veuillez contacter l\'administrateur pour toute modification.'
                ]);
                exit;
            }
        }
