...
/* ── FILIGRANE LOGO (désactivé - remplacé par logos latéraux) ── */
body::before{
  content:'';
  position:fixed;
  inset:0;
...
.login-side-logo{
  ...
  border-radius:50%;
  flex-shrink:0;
  background:url('Logo_CCPD.png') center center / contain no-repeat;
  opacity:.18;
...
<div class="login-wrap">
  ...
  <div class="login-center-row">
    ...
    <!-- Logo gauche (filigrane) -->
    <div class="login-side-logo" role="img" aria-label="Logo CCPD"></div>
...
function openModal(td){
  ...
  // Bloquer si un congé admin (non-PREV) est déjà posé — même pour l'agent lui-même
  if(!IS_ADMIN && curCongeId>0){
    const typeExistant=td.dataset.congeType||'';
    if(typeExistant !== '' && typeExistant !== 'PREV'){
      showToast('⛔ Un congé ('+typeExistant+') a déjà été posé sur ce jour par l\'administrateur. Veuillez le contacter pour toute modification.',false);return;
    }
  }
