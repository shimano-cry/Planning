...
.wrap-table td,.wrap-table th{width:42px;overflow:hidden;text-overflow:ellipsis;font-size:.68rem!important}
...
function openCongeModal(td,date,groupe,cycle,isPerm){
  ...
  if(isPerm){
    ...
    // Indisponibilités : toujours visibles
    ...
    if(permGrid){
      if(isEquipePerm){
        // Équipe/Standard : M AM sur ligne 1 (span 3 chacun) + IM IAM IJ sur ligne 2
        ...
        // Tous les autres : uniquement 3 indispos
  ...
  ensureQuotaCache(); // Précharger quota en arrière-plan
    ...
    document.getElementById('sec-periode').style.display='block';
...
function buildGrid(g){
  ...
  if(GROUPES_SPECIAUX.includes(g)){
    ...
    if(g==='lcl_parent'||g==='adc_lambert'){
      ...
      document.querySelector('.modal').style.width='360px';
...
async function loadCongesRecap(){
  ...
  // Bordures : fine entre cellules du même type, épaisse entre types
  const SEP_STYLE    = 'border-right:3px solid #1a2742;';
  const INNER_BORDER = 'border-right:1px solid #d0d8f0;';

  let html = `<div style="overflow-x:auto">
  <table style="border-collapse:collapse;font-size:11.5px;width:100%;border:2px solid #1a2742">
  <thead>
  ...
  TYPES.forEach((t)=>{
    html+=`<th style="padding:5px 7px;white-space:nowrap;${SEP_STYLE}" colspan="3">${CR_TYPE_LBL[t]||t}</th>`;
  });
  html+=`</tr>
  ...
  TYPES.forEach(()=>{
    html+=`<th style="padding:3px 5px;${INNER_BORDER}">Dotation</th><th style="padding:3px 5px;${INNER_BORDER}">Posé</th><th style="padding:3px 5px;${SEP_STYLE}">Reste</th>`;
  });
  html+=`</tr></thead><tbody>`;

  CR_GROUPES.forEach(({lbl, agents})=>{
    html+=`<tr style="background:#e8eef6">
      <td colspan="${1+TYPES.length*3}" style="padding:4px 10px;font-size:10.5px;font-weight:800;color:#1a2742;letter-spacing:.04em;text-transform:uppercase">${lbl}</td>
    </tr>`;
    agents.forEach((ag,i)=>{
      const agCounts  = counts[ag]  || {};
      ...
      TYPES.forEach((t,idx)=>{
        ...
        html+=`<td style="text-align:center;padding:4px 6px;color:#555;font-weight:600;${INNER_BORDER}">${dotFmt}</td>`;
        html+=`<td style="text-align:center;padding:4px 6px;color:${posCol};font-weight:${used>0?'700':'400'};${INNER_BORDER}">${used>0?usedFmt:'—'}</td>`;
