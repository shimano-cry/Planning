...
function renderCellsHtml(mysqli $db, array $agentsList, array $datesList,
    ...
    $gendaCouleurs = [
        ...
        'R' => ['couleur_bg'=>'#CCFFCC','couleur_txt'=>'#000000'],
    ];
    ...
    foreach ($agentsList as $ag) {
        ...
        foreach ($datesList as $dt) {
            ...
            if ($annId > 0) {
                ...
                echo "<td class='$clsCg$lockCls'$da$styleCg data-ferie='0' data-conge-id='$cgId2' data-conge-type='$cgCode' data-conge-valide='".($meta['valide']??0)."' data-perm-id='0' data-tir-id='0' data-conge-per='$per' data-conge-heure='".htmlspecialchars($hre)."' title='".htmlspecialchars($tip)."'>$txt</td>";
...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_SERVER['HTTP_X_REQUESTED_WITH'])) {
    ...
    if (in_array($action,['save_conge','delete_conge','save_perm','delete_perm','save_tir','delete_tir'])) {
        ...
        if (in_array($action,['save_perm','delete_perm']) && !$isAdmin && !($userRights['can_perm']??false)) {
            ...
            // CoGe ROUSSEL : autorisé sur FERIE/RC/RL avec M/AM/NUIT/J
            if ($isCoGe && $isFerieWE) { /* autorisé → laisser passer */ }
            elseif (!$isFerieWE) {
        ...
        if (in_array($action,['save_conge','delete_conge']) && !$isAdmin && !($userRights['can_conge']??false) && !$isDouane && !$isCoGe) {
            // PREV, DA, PR autorisés à tous les utilisateurs authentifiés
            $allowedUserTypes = ['PREV','DA','PR'];
    ...
    /* ── Plan de rappel ── */
        ...
        $res=$mysqli->query("SELECT id,priorite,nom,telephone,remarque FROM plan_rappel ORDER BY priorite,nom");
        $rows=[];
        while($r=$res->fetch_assoc()) $rows[]=$r;
        echo json_encode(['ok'=>true,'rows'=>$rows]);
        ...
        if($rid>0){
            $st=$mysqli->prepare("UPDATE plan_rappel SET priorite=?,nom=?,telephone=?,remarque=? WHERE id=?");
            $st->bind_param('isssi',$priorite,$nom,$tel,$rem,$rid);
        } else {
            $st=$mysqli->prepare("INSERT INTO plan_rappel (priorite,nom,telephone,remarque) VALUES (?,?,?,?)");
            $st->bind_param('isss',$priorite,$nom,$tel,$rem);
        }
        $ok=$st->execute();
...
function loadConges(mysqli $db,int $annee,int $moisD,int $moisF): array {
    ...
    $gendaCouleursFix = [
        ...
        'R' => ['couleur_bg'=>'#CCFFCC','couleur_txt'=>'#000000','libelle'=>'Repos'],
    ];
    foreach (['LCL PARENT','ADC LAMBERT'] as $gendaAg) {
...
$codesAdcLambert=[
  ...
  'R'=>['code'=>'R','libelle'=>'Repos',     'couleur_bg'=>'#CCFFCC','couleur_txt'=>'#000000'],
];
...
function ligne(string $agent,callable $cb,bool $masqueRC,bool $ignFe=false,string $grp='standard'): void {
  ...
  for ($d=1;$d<=$nbJours;$d++) {
    ...
    if ($tirPer !== null) {
      ...
      echo "<td class='$clsCg2$lockCls'$da$styleCg2 data-ferie='0' data-conge-id='$cgId' data-conge-type='$cgCode' data-conge-valide='".($meta['valide']??0)."' data-perm-id='0' data-tir-id='0' data-conge-per='$per' data-conge-heure='".htmlspecialchars($hre)."' title='".htmlspecialchars($tip)."'>$cellTxt</td>";
...
.wrap-table table{table-layout:fixed}
.wrap-table td,.wrap-table th{width:52px;overflow:hidden;text-overflow:ellipsis;font-size:.68rem!important}
.wrap-table td.agent-name,.wrap-table th:first-child{width:130px;min-width:130px}
...
.code-btn{border:2px solid transparent;border-radius:6px;padding:5px 4px;cursor:pointer;
  font-family:'Arial',sans-serif;font-weight:700;
  font-size:.78rem;line-height:1.2;transition:transform .1s,box-shadow .12s;text-align:center}
...
.code-btn small{display:block;font-size:.58rem;font-weight:400;margin-top:2px;white-space:normal;line-height:1.2}
...
<div class="toolbar">
  ...
  <button class="btn-action" id="btn-open-rappel" style="background:#c0392b;color:#fff;border-color:#c0392b">📞 Rappel</button>
    ...
    <div class="modal-body">
      ...
      <div id="sec-perm" style="display:none">
        ...
        <div class="perm-grid" id="perm-grid" style="grid-template-columns:repeat(3,1fr);gap:6px">
          ...
          <!-- Indisponibilités supprimées - uniquement M et AM -->
        </div>
      </div>
...
<div class="overlay-perm" id="overlay-rappel">
  <div class="modal-perm" style="max-width:700px">
...
const ADMIN_CONGE_CODES=[
  '__SEP_NORMAL__',
  'CAA','CA','HPA','HP','HS','RTT',
  'PR','DA','__EMPTY__','CF','RPS','CET',
  '__SEP_SPECIAL__',
  'CMO','CLM','CLD','BS','SBS','ACC',
  'ASA','AT','GEM','AUT','__EMPTY__','PREV'
];
...
function openModal(td){
  ...
  const isPerm=!isGieSemaine&&!isDouaneAgent&&(hasPermOk
    ||(isFerieOrWE&&(IS_ADMIN||IS_COGE||AGENTS_PERM.includes(td.dataset.agent))
       &&!(isFerie&&!GROUPES_PERM_FERIE.includes(td.dataset.groupe)&&!IS_ADMIN&&!IS_COGE)));
...
function openCongeModal(td,date,groupe,cycle,isPerm){
  ...
  if(isPerm){
    ...
    if(btnPermM)  btnPermM.style.display  = isEquipePerm ? 'inline-block' : 'none';
    if(btnPermAM) btnPermAM.style.display = isEquipePerm ? 'inline-block' : 'none';

    // Grille : M et AM uniquement
    ...
    if(permGrid){
      if(isEquipePerm){
        ...
        if(btnPermM)  { btnPermM.style.gridColumn='span 1'; }
        if(btnPermAM) { btnPermAM.style.gridColumn='span 1'; }
      } else {
        permGrid.style.gridTemplateColumns='repeat(1,1fr)';
        modal.style.width='400px';
      }
      ['perm-m','perm-am'].forEach(id=>{
...
function buildGrid(g){
  ...
  if(GROUPES_SPECIAUX.includes(g)){
    ...
    if(g==='lcl_parent'||g==='adc_lambert'){
      ...
      types=types.map(t=>t.code==='R'?{...t,couleur_bg:'#bfbfbf',couleur_txt:'#333'}:t);
    } else if(g==='direction_police'){
      const ordered=ADMIN_CONGE_CODES
        .map(code=>{
          if(code==='__EMPTY__') return null;
          if(code==='__SEP_NORMAL__'||code==='__SEP_SPECIAL__') return code;
      ...
      const GIE_COLORS={'M':{bg:'#ffd200',txt:'#222'},'AM':{bg:'#2f5597',txt:'#ffc000'},'J':{bg:'#92d050',txt:'#222'},'P':{bg:'#bfbfbf',txt:'#333'},'R':{bg:'#92d050',txt:'#222'}};
  ...
  types.forEach(t=>{
    ...
    // Séparateurs titrés — s'étendent sur toute la largeur de la grille
    if(t==='__SEP_NORMAL__'||t==='__SEP_SPECIAL__'){
      const sep=document.createElement('div');
      sep.style.cssText='grid-column:1/-1;margin:6px 0 4px;padding:4px 8px;'
        +'background:'+(t==='__SEP_NORMAL__'?'#1a2742':'#6a1b9a')
        +';color:#fff;font-size:.72rem;font-weight:700;border-radius:5px;letter-spacing:.04em';
      if(t==='__SEP_SPECIAL__'){
        // Séparateur cliquable avec toggle
        const SPECIAL_CODES=['ASA','CAM','CONV','GEM','VEM','STG','CMO','CLM','CLD','AUT','PREV'];
        const isSpecialSelected=SPECIAL_CODES.includes(selType);
        let specialOpen=isSpecialSelected;
        // Créer le wrapper des boutons spéciaux (sous-grille hors du flux principal)
        const specialWrap=document.createElement('div');
        specialWrap.style.cssText='grid-column:1/-1;display:'+(specialOpen?'grid':'none')
          +';grid-template-columns:repeat(6,1fr);gap:4px;margin-bottom:4px';
        specialWrap.id='special-codes-wrap';
        sep.style.cursor='pointer';
        sep.style.userSelect='none';
        const arrow=document.createElement('span');
        arrow.style.cssText='float:right;font-size:.8rem;transition:transform .2s';
        arrow.textContent=specialOpen?'▲':'▼';
        sep.textContent='Absences spéciales ';
        sep.appendChild(arrow);
        sep.addEventListener('click',()=>{
          specialOpen=!specialOpen;
          specialWrap.style.display=specialOpen?'grid':'none';
          arrow.textContent=specialOpen?'▲':'▼';
        });
        codeGrid.appendChild(sep);
        // Stocker le wrapper pour y ajouter les boutons spéciaux
        codeGrid._specialWrap=specialWrap;
        codeGrid._inSpecial=true;
        codeGrid.appendChild(specialWrap);
      } else {
        sep.textContent='Congés normaux';
        codeGrid._inSpecial=false;
        codeGrid.appendChild(sep);
      }
      return;
    ...
    btn.innerHTML=t.code+'<small style="font-size:.58rem;white-space:normal;overflow:visible;text-overflow:clip;line-height:1.2">'
      +t.libelle+'</small>';
...
document.addEventListener('keydown',e=>{if(e.key==='Escape'){closeModal();closePerm();overlayTir.classList.remove('open');overlayRecapFetes.classList.remove('open');}});
...
// Boutons permanence
['perm-m','perm-am','perm-nuit','perm-j'].forEach(id=>{
  document.getElementById(id)?.addEventListener('click',()=>{
...
async function loadRappel(){
  ...
  // Tableau
  if(r.rows.length){
    html+=`<table style="border-collapse:collapse;width:100%;font-size:.82rem;margin-bottom:12px">
    <thead><tr style="background:#1a2742;color:#fff">
      <th style="padding:6px 10px;text-align:center;width:60px">Priorité</th>
      <th style="padding:6px 10px;text-align:left">Nom</th>
      <th style="padding:6px 10px;text-align:left">Téléphone</th>
      <th style="padding:6px 10px;text-align:left">Remarque</th>
      ${IS_ADMIN?'<th style="padding:6px 10px;text-align:center">Actions</th>':''}
    </tr></thead><tbody>`;
    r.rows.forEach((row,i)=>{
      html+=`<tr style="background:${i%2?'#f9f9f9':'#fff'}">
        <td style="padding:5px 10px;text-align:center;font-weight:700;color:#c0392b">${row.priorite||'—'}</td>
        <td style="padding:5px 10px;font-weight:700;color:#1a2742">${row.nom}</td>
        <td style="padding:5px 10px"><a href="tel:${row.telephone.replace(/\s/g,'')}" style="color:#1565c0;font-weight:700;text-decoration:none">${row.telephone||'—'}</a></td>
        <td style="padding:5px 10px;color:#555;font-style:italic">${row.remarque||''}</td>
        ${IS_ADMIN?`<td style="padding:5px 10px;text-align:center;white-space:nowrap">
          <button class="btn-edit-rappel" data-id="${row.id}" data-priorite="${row.priorite}" data-nom="${row.nom}" data-tel="${row.telephone}" data-rem="${row.remarque||''}"
            style="padding:2px 8px;background:#1565c0;color:#fff;border:none;border-radius:4px;font-size:.72rem;cursor:pointer;margin-right:3px">✏️</button>
          <button class="btn-del-rappel" data-id="${row.id}"
            style="padding:2px 8px;background:#c0392b;color:#fff;border:none;border-radius:4px;font-size:.72rem;cursor:pointer">🗑️</button>
        </td>`:''}
      </tr>`;
    });
    html+=`</tbody></table>`;
  ...
  if(IS_ADMIN){
    html+=`<div id="rappel-form" style="border:1.5px solid #e0e0e0;border-radius:8px;padding:12px;background:#f9f9f9;margin-top:8px">
      ...
      <div style="display:grid;grid-template-columns:80px 1fr 1fr 1fr auto;gap:8px;align-items:end">
        <div><label style="font-size:.72rem;color:#666">Priorité</label><br>
          <input id="rappel-priorite" type="number" min="0" max="99" value="0" style="width:100%;padding:4px 6px;border:1.5px solid #ccc;border-radius:5px;font-size:.78rem"></div>
        ...
        <div><label style="font-size:.72rem;color:#666">Téléphone</label><br>
          <input id="rappel-tel" type="text" placeholder="06 xx xx xx xx" style="width:100%;padding:4px 6px;border:1.5px solid #ccc;border-radius:5px;font-size:.78rem"></div>
        <div><label style="font-size:.72rem;color:#666">Remarque</label><br>
          <input id="rappel-rem" type="text" placeholder="Remarque" style="width:100%;padding:4px 6px;border:1.5px solid #ccc;border-radius:5px;font-size:.78rem"></div>
  ...
  if(IS_ADMIN){
    body.querySelectorAll('.btn-edit-rappel').forEach(btn=>{
      btn.addEventListener('click',()=>{
        ...
        document.getElementById('rappel-priorite').value=btn.dataset.priorite;
        document.getElementById('rappel-nom').value=btn.dataset.nom;
        document.getElementById('rappel-tel').value=btn.dataset.tel;
        document.getElementById('rappel-rem').value=btn.dataset.rem;
      });
    });
    ...
    document.getElementById('btn-save-rappel')?.addEventListener('click',async()=>{
      ...
      const r=await ajax({
        ...
        priorite:document.getElementById('rappel-priorite').value,
        nom,telephone:document.getElementById('rappel-tel').value,
        remarque:document.getElementById('rappel-rem').value
      });
    ...
    document.getElementById('btn-cancel-rappel')?.addEventListener('click',()=>{
      ...
      document.getElementById('rappel-priorite').value='0';
      document.getElementById('rappel-nom').value='';
      document.getElementById('rappel-tel').value='';
      document.getElementById('rappel-rem').value='';
    });
  }
  ...
  document.getElementById('btn-print-rappel')?.addEventListener('click',()=>{
    ...
    w.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Plan de rappel</title>
      <style>body{font-family:Arial,sans-serif;margin:10mm;font-size:11px}table{border-collapse:collapse;width:100%}th,td{padding:4px 8px;border:1px solid #ccc}th{background:#1a2742;color:#fff}@page{size:A4;margin:10mm}*{-webkit-print-color-adjust:exact;print-color-adjust:exact}</style>
    </head><body><h3 style="font-size:13px;color:#1a2742;margin-bottom:8px">📞 Plan de rappel</h3>${tbl.outerHTML}</body></html>`);
...
async function loadValidations(){
  ...
  wrap.querySelectorAll('.btn-refuser').forEach(btn=>{
    btn.addEventListener('click',async()=>{
      ...
      if(r.ok){ if(r.cells&&r.cells.length) applyCells(r); loadValidations(); }
    });
  });
}
