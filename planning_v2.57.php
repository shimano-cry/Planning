...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_SERVER['HTTP_X_REQUESTED_WITH'])) {
    ...
    /* ── Plan de rappel ── */
        ...
        $res=$mysqli->query("SELECT id,nom,prenom,tel_domicile,tel_gsm,tel_neo,fonction FROM plan_rappel ORDER BY nom,prenom");
        $rows=[];
        while($r=$res->fetch_assoc()) $rows[]=$r;
        echo json_encode(['ok'=>true,'rows'=>$rows]);
        ...
        if($rid>0){
            $st=$mysqli->prepare("UPDATE plan_rappel SET nom=?,prenom=?,tel_domicile=?,tel_gsm=?,tel_neo=?,fonction=? WHERE id=?");
            $st->bind_param('ssssssi',$nom,$prenom,$dom,$gsm,$neo,$fct,$rid);
        } else {
            $st=$mysqli->prepare("INSERT INTO plan_rappel (nom,prenom,tel_domicile,tel_gsm,tel_neo,fonction) VALUES (?,?,?,?,?,?)");
            $st->bind_param('ssssss',$nom,$prenom,$dom,$gsm,$neo,$fct);
        }
        $ok=$st->execute();
...
.wrap-table td,.wrap-table th{width:42px;min-width:42px;overflow:hidden;text-overflow:ellipsis;font-size:.62rem!important}
.wrap-table td.agent-name,.wrap-table th:first-child{width:110px;min-width:110px}
/* Bold uniquement sur congés et absences */
.wrap-table td.conge,.wrap-table td.prev-conge{font-weight:800!important;font-size:.60rem!important}
.wrap-table td.tir{font-weight:700!important;font-size:.60rem!important}
...
async function loadRappel(){
  ...
  if(r.rows.length){
    ...
    <thead><tr style="background:#1a2742;color:#fff">
      ...
      <th style="padding:6px 10px;text-align:left">Domicile</th>
      <th style="padding:6px 10px;text-align:left">GSM / Portable</th>
      <th style="padding:6px 10px;text-align:left">NEO</th>
      <th style="padding:6px 10px;text-align:left">Fonction / Rôle</th>
      ${IS_ADMIN?'<th style="padding:6px 10px;text-align:center">Actions</th>':''}
    </tr></thead><tbody>`;
    r.rows.forEach((row,i)=>{
      const tel=(t)=>t?`<a href="tel:${t.replace(/\s/g,'')}" style="color:#1565c0;font-weight:700;text-decoration:none">${t}</a>`:'—';
      html+=`<tr style="background:${i%2?'#f9f9f9':'#fff'}">
        <td style="padding:5px 10px;font-weight:700;color:#c0392b">${row.prenom||'—'}</td>
        <td style="padding:5px 10px;font-weight:700;color:#1a2742">${row.nom}</td>
        <td style="padding:5px 10px">${tel(row.tel_domicile)}</td>
        <td style="padding:5px 10px">${tel(row.tel_gsm)}</td>
        <td style="padding:5px 10px">${tel(row.tel_neo)}</td>
        <td style="padding:5px 10px;color:#555;font-style:italic">${row.fonction||''}</td>
        ${IS_ADMIN?`<td style="padding:5px 10px;text-align:center;white-space:nowrap">
          <button class="btn-edit-rappel" data-id="${row.id}" data-nom="${row.nom}" data-prenom="${row.prenom||''}" data-dom="${row.tel_domicile||''}" data-gsm="${row.tel_gsm||''}" data-neo="${row.tel_neo||''}" data-fct="${row.fonction||''}"
  ...
  if(IS_ADMIN){
    html+=`<div id="rappel-form" style="border:1.5px solid #e0e0e0;border-radius:8px;padding:12px;background:#f9f9f9;margin-top:8px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr 1fr 1fr auto;gap:8px;align-items:end">
