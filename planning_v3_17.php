...
(function(){
  ...
  function getLabel(td){
    ...
    if(congeType&&congeType!==''){
      ...
      if(conge2Type){
        // Double congé demi-journée : pas d'heure dans ce cas
        lines.push('📅 ½ '+congeType+' — Matin');
        lines.push('📅 ½ '+conge2Type+' — Après-midi');
      } else {
        const sym=CONGE_SYM[per]||'';
        // Pour DA/PR : afficher l'heure dans le tooltip (pas dans la cellule)
    ...
    while ($c = $res->fetch_assoc()) {
        ...
        while ($d <= $fi) {
            ...
            if (in_array($ds, $datesList)) {
                $entry = ['periode'=>$c['periode']??'J','libelle'=>$c['libelle'],'heure'=>$c['heure']??'','couleur_bg'=>$c['couleur_bg']??'#1565c0','couleur_txt'=>$c['couleur_txt']??'#ffffff','valide'=>(int)($c['valide']??0),'type_conge'=>$c['type_conge'],'id'=>$c['id']];
    ...
    foreach ($agentsList as $ag) {
        ...
        foreach ($datesList as $dt) {
            ...
            if ($annId > 0) {
                ...
                echo "<td class='$clsCg$lockCls$valideCls'$da$styleCg data-ferie='0' data-conge-id='$cgId2' data-conge-type='$cgCode' data-conge-valide='$valide' data-perm-id='0' data-tir-id='0' data-conge-per='$per' data-conge-heure='".htmlspecialchars($hre)."'$dataConge2>$txt</td>";
...
function ligne(string $agent,callable $cb,bool $masqueRC,bool $ignFe=false,string $grp='standard'): void {
  ...
  for ($d=1;$d<=$nbJours;$d++) {
    ...
    if ($tirPer !== null) {
      ...
      echo "<td class='$clsCg2$lockCls$valideCls2'$da$styleCg2 data-ferie='0' data-conge-id='$cgId' data-conge-type='$cgCode' data-conge-valide='$valide2' data-perm-id='0' data-tir-id='0' data-conge-per='$per' data-conge-heure='".htmlspecialchars($hre)."'$dataConge2L>$cellTxt</td>";
...
async function loadRappel(){
  ...
  document.getElementById('btn-print-rappel')?.addEventListener('click',()=>{
    ...
    if(tblClone){
      ...
      // Supprimer le dernier <td> de chaque ligne (boutons actions)
      tblClone.querySelectorAll('tbody tr').forEach(tr=>{
        const thCount=tblClone.querySelectorAll('thead tr th').length;
        if(tr.querySelectorAll('td').length > thCount){
          tr.querySelector('td:last-child').remove();
        }
      });
      // Supprimer le colgroup (table-layout:auto gère les largeurs seul)
