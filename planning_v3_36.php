...
runMigrationOnce($mysqli, 'update_plan_rappel_v7', function(mysqli $db) {
    $db->query("ALTER TABLE plan_rappel ADD COLUMN IF NOT EXISTS pays CHAR(2) NOT NULL DEFAULT 'FR'");
    // Agents belges connus : BOUXOM (GSM BE), DRUEZ (domicile BE) — on les passe à BE
    $db->query("UPDATE plan_rappel SET pays='BE' WHERE nom IN ('BOUXOM','DRUEZ')");
});
/* ======================
   AUTHENTIFICATION
====================== */
...
if ($isLogged && !$isAdmin) {
    ...
    // Gendarmerie : can_tir si au moins un de leurs agents est dans user_agents
    $gendarmerieAgents = ['LCL PARENT','ADJ CORRARD','ADJ LEFEBVRE','ADC LAMBERT'];
    if (!empty(array_intersect($userAgents, $gendarmerieAgents))) {
        $userRights['can_tir'] = true;
    }
...
(function(){
  ...
  const CYCLE_LABELS={
    ...
    'j':'Journée','m':'Matin','am':'Après-midi',
    'P':'Permission','R':'Repos',
  ...
  function getLabel(td){
    ...
    if(congeType&&congeType!==''){
      const conge2Type=td.dataset.conge2Type||'';
      // P, R, J, M, AM sont des codes GIE enregistrés comme congés
      const GIE_CONGE_LABELS={'P':'Permission','R':'Repos','J':'Journée','M':'Matin','AM':'Après-midi'};
      if(GIE_CONGE_LABELS[congeType]){
        lines.push('📋 '+GIE_CONGE_LABELS[congeType]);
      } else if(conge2Type){
        lines.push('📅 ½ '+congeType+' — Matin');
        lines.push('📅 ½ '+conge2Type+' — Après-midi');
      } else {
        const sym=CONGE_SYM[per]||'';
        const isDA=(congeType==='DA'||congeType==='PR');
        const hLbl=(heure&&isDA)?' à '+heure.replace(':','h'):'';
...
// Agents AUTORISÉS TIR : police + gendarmerie
// Exclus douane : IR MOREAU, ACP1 DEMERVAL, ACP1 LOIN
// Exclus autres : AA MAES (secrétariat)
// Gendarmerie (PARENT, CORRARD, LEFEBVRE, LAMBERT) : TIR autorisé Matin/AM uniquement
$agentsTir = array_values(array_diff(
  ['CoGe ROUSSEL','Cne MOKADEM',
   ...
   // Gendarmerie — TIR Matin/AM uniquement
   'LCL PARENT','ADJ CORRARD','ADJ LEFEBVRE','ADC LAMBERT'],
  $agentsExclusTir
));
// Agents Gendarmerie TIR : pas de période Nuit
$agentsTirGendarmerie = ['LCL PARENT','ADJ CORRARD','ADJ LEFEBVRE','ADC LAMBERT'];
...
function renderCellsHtml(mysqli $db, array $agentsList, array $datesList,
    ...
    while ($c = $res->fetch_assoc()) {
        ...
        while ($d <= $fi) {
            ...
            if (in_array($ds, $datesList)) {
                $entry = ['periode'=>$c['periode']??'J','libelle'=>$c['libelle'],'heure'=>$c['heure']??'','couleur_bg'=>$c['couleur_bg']??'#1565c0','couleur_txt'=>$c['couleur_txt']??'#ffffff','valide'=>(int)($c['valide']??0),'type_conge'=>$c['type_conge'],'id'=>$c['id'],'date_debut'=>$c['date_debut'],'date_fin'=>$c['date_fin']];
                // Premier congé du jour : initialiser
                if (!isset($cgMeta[$a][$ds])) {
                    $cg[$a][$ds]       = $c['type_conge'];
                    $cgId[$a][$ds]     = $c['id'];
                    $cgMeta[$a][$ds]   = $entry;
                    $cgAll[$a][$ds]    = [$entry];
                } else {
                    // Second congé (demi-journée complémentaire) : stocker les deux
                    $cgAll[$a][$ds][]  = $entry;
                    // Priorité d'affichage : M prime sur AM pour la cellule principale
                    if (($entry['periode']??'J') === 'M') {
                        $cg[$a][$ds]   = $c['type_conge'];
                        $cgId[$a][$ds] = $c['id'];
                        $cgMeta[$a][$ds] = $entry;
                    }
    ...
    foreach ($agentsList as $ag) {
        ...
        $masq = in_array($grp, ['standard_police','equipe','gie']); // nuit exclu : RC/RL visibles
        ...
        foreach ($datesList as $dt) {
            ...
            if ($annId > 0) {
                ...
                $tip    = match($pmType){'M'=>'Permanence Matin','AM'=>'Permanence Après-midi','IM'=>'Indisponibilité Matin','IAM'=>'Indisponibilité Après-midi','IJ'=>'Indisponibilité Journée','P'=>'Permission','R'=>'Repos',default=>$pmType};
                echo "<td class='$clsPm perm-ok$permLocked$lockCls'$da data-ferie='0' data-conge-id='0' data-conge-type='' data-perm-id='$pmId2' data-perm-type='$pmType' data-cycle-orig='$pmCyc' data-tir-id='0'>$disp$sym</td>";

            } elseif ($cgCode && !($isFe && $masq)) {
                $meta  = $cgMeta[$ag][$dt] ?? ['periode'=>'J','libelle'=>$cgCode,'heure'=>'','couleur_bg'=>'#1565c0','couleur_txt'=>'#ffffff'];
                $per   = $meta['periode'] ?? 'J';
                $hre   = $meta['heure']   ?? '';
                // Détection double congé (M + AM sur même jour)
                $allCgs = $cgAll[$ag][$dt] ?? [$meta + ['type_conge'=>$cgCode]];
                $isDouble = count($allCgs) >= 2 && $cycleVal === 'J';
                if ($isDouble) {
                    usort($allCgs, fn($a,$b) => ($a['periode']==='M' ? 0 : 1) - ($b['periode']==='M' ? 0 : 1));
                    $cg1 = $allCgs[0]; $cg2 = $allCgs[1];
                    $code1 = $cg1['type_conge']??$cgCode; $code2 = $cg2['type_conge']??$cgCode;
                    $lib1  = $cg1['libelle'] ?: $code1;   $lib2  = $cg2['libelle'] ?: $code2;
                    $txt   = "$code1+$code2"; // résumé court dans la cellule
                    $tip   = "½ $lib1 (Matin) + ½ $lib2 (Après-midi)";
                    // data-conge2 pour le tooltip JS
                    $id2 = $allCgs[1]['id'] ?? 0;
                    $dataConge2 = " data-conge2-type='".htmlspecialchars($code2)."' data-conge2-per='AM' data-conge2-id='$id2'";
                    $tip = ''; // title= vide : le cell-tip JS gère l'affichage
                } else {
                    $dataConge2 = '';
                    $sym   = match($per){'M'=>'☀','AM'=>'🌙',default=>''};
                    $txt   = $cgCode.$sym; // heure retirée de la cellule pour DA/PR
                    $tip   = $meta['libelle'] ?: $cgCode;
                    if ($hre && in_array($cgCode,['DA','PR'])) $tip .= ' à '.substr($hre,0,5); // heure dans tooltip
                ...
                $prevData = ($cgCode==='PREV') ? " data-prev-debut='".htmlspecialchars($meta['date_debut']??$dt)."' data-prev-fin='".htmlspecialchars($meta['date_fin']??$dt)."'" : '';
                echo "<td class='$clsCg$lockCls$valideCls'$da$styleCg data-ferie='0' data-conge-id='$cgId2' data-conge-type='$cgCode' data-conge-valide='$valide' data-perm-id='0' data-tir-id='0' data-conge-per='$per' data-conge-heure='".htmlspecialchars($hre)."'$dataConge2$prevData>$txt</td>";

            } elseif ($isFe && !$masq) {
                $feriePermOkAjax = $pP ? ' perm-ok' : '';
                echo "<td class='ferie$feriePermOkAjax$lockCls'$da data-ferie='1' data-conge-id='0' data-conge-type='' data-perm-id='0' data-cycle-orig='FERIE' data-tir-id='0' title='Jour férié'>FERIE</td>";

            } elseif ($isFe && $masq) {
                // Groupes cycliques (nuit/equipe/gie) : afficher le cycle même sur férié
                $val = $cycleVal;
                if (in_array($val, ['RC','RL'])) {
                    $cls  = $pP ? 'rc-masque perm-ok' : 'rc-masque';
                    $tipM = $val === 'RC' ? 'Repos compensatoire' : 'Repos légal (dimanche)';
                    echo "<td class='$cls$lockCls'$da data-ferie='1' data-conge-id='0' data-conge-type='' data-perm-id='0' data-cycle-orig='$val' data-masque='1' data-tir-id='0' title='".htmlspecialchars($tipM)."'></td>";
                } else {
                    $cls = cb($val);
                    if ($pP && in_array($val,['RC','RL'])) $cls .= ' perm-ok';
                    $tipV = match($val){'NUIT'=>'Nuit','M'=>'Matin','AM'=>'Après-midi','J'=>'Journée',default=>$val};
                    echo "<td class='$cls$lockCls'$da data-ferie='1' data-conge-id='0' data-conge-type='' data-perm-id='0' data-cycle-orig='$val' data-tir-id='0' title='".htmlspecialchars($tipV)."'>$val</td>";
                }
...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_SERVER['HTTP_X_REQUESTED_WITH'])) {
    ...
    if ($action==='save_conge') {
        ...
        if ($conge_id > 0) {
            ...
            $dateFinEsc  = $mysqli->real_escape_string($date_fin);
            $deletedRows = 0;
            // Cas demi-journée sur une seule date : ne pas supprimer l'autre demi-journée complémentaire
            $isDemiJour  = ($date_debut === $date_fin && in_array($periode, ['M','AM']));
            if ($isDemiJour) {
                $periodeCompl = ($periode === 'M') ? 'AM' : 'M';
                // Supprimer uniquement les congés qui chevauchent ET ont la même période (ou sont journée entière)
                $sqlDel = "DELETE FROM conges WHERE agent='$agentEsc'
                           AND date_debut<='$dateFinEsc' AND date_fin>='$dateDebEsc'
                           AND (periode='$periode' OR periode='J' OR periode IS NULL OR periode='')";
            } else {
                // Cas normal (journée entière ou plage) : supprimer tout ce qui chevauche
        ...
        // Rafraîchir aussi les autres agents du même groupe (cycle partagé)
        $grpAgent=$groupeAgent[$agent]??'';
        $agentsToRefresh=[$agent];
        if(in_array($grpAgent,['nuit','equipe','gie','gie_j'])){
            foreach($groupeAgent as $ag=>$grp){
                if($grp===$grpAgent && $ag!==$agent) $agentsToRefresh[]=$ag;
            }
            // Élargir aux dates du mois complet car le cycle affecte toutes les cellules
            $moisRef=substr($date_debut,0,7); // YYYY-MM
            $dts=[];
            $dm=new DateTime($moisRef.'-01');
            $fm=new DateTime($moisRef.'-'.date('t',strtotime($moisRef.'-01')));
            while($dm<=$fm){$dts[]=$dm->format('Y-m-d');$dm->modify('+1 day');}
        }
        $cells=renderCellsHtml($mysqli,$agentsToRefresh,$dts,$groupeAgent,$agentsTir,$agentsPermanence,$isAdmin,$globalLocked,$locks,$userAgents);
        echo json_encode(['ok'=>true,'msg'=>'Congé sauvegardé','id'=>$nid,'date_debut'=>$date_debut,'date_fin'=>$date_fin,'cells'=>$cells]);

    /* ── Scission CONGÉ (retirer un jour d'une plage PREV) ── */
    } elseif ($action==='split_conge') {
        $conge_id  = (int)($_POST['conge_id'] ?? 0);
        $date_split= trim($_POST['date_split'] ?? '');
        if (!$conge_id || !preg_match($dateReg, $date_split))
            { echo json_encode(['ok'=>false,'msg'=>'Paramètres invalides']); exit; }
        // Récupérer la plage originale
        $st = $mysqli->prepare("SELECT agent,date_debut,date_fin,type_conge,demi_jour,periode,heure FROM conges WHERE id=?");
        $st->bind_param('i', $conge_id);
        $st->execute();
        $row = $st->get_result()->fetch_assoc();
        if (!$row) { echo json_encode(['ok'=>false,'msg'=>'Congé introuvable']); exit; }
        // Récupérer la plage à retirer (début et fin)
        $date_split_fin = trim($_POST['date_split_fin'] ?? $date_split);
        if (!preg_match($dateReg, $date_split_fin)) $date_split_fin = $date_split;
        if ($date_split_fin < $date_split) $date_split_fin = $date_split;
        // Vérifier que la plage à retirer est dans la plage originale
        if ($date_split < $row['date_debut'] || $date_split_fin > $row['date_fin'])
            { echo json_encode(['ok'=>false,'msg'=>'Plage hors des limites du congé original']); exit; }
        // Droits : non-admin ne peut scinder que ses propres PREV
        if (!$isAdmin && $row['type_conge'] !== 'PREV')
            { echo json_encode(['ok'=>false,'msg'=>"Action réservée à l'administrateur pour ce type de congé"]); exit; }
        // Supprimer le congé original
        $stDel = $mysqli->prepare("DELETE FROM conges WHERE id=?");
        $stDel->bind_param('i', $conge_id);
        $stDel->execute();
        $deb  = new DateTime($row['date_debut']);
        $fin  = new DateTime($row['date_fin']);
        $splitDeb = new DateTime($date_split);
        $splitFin = new DateTime($date_split_fin);
        $prev1_fin = (clone $splitDeb)->modify('-1 day');
        $prev2_deb = (clone $splitFin)->modify('+1 day');
        $dts = [];
        // INSERT plage 1 : début original → veille du début retiré
        if ($deb <= $prev1_fin) {
            $d1 = $deb->format('Y-m-d'); $f1 = $prev1_fin->format('Y-m-d');
            $st1 = $mysqli->prepare("INSERT INTO conges (agent,agent_nom,date_debut,date_fin,type,type_conge,demi_jour,periode,heure) VALUES (?,?,?,?,?,?,?,?,?)");
            $tc=$row['type_conge']; $dj=$row['demi_jour']; $pe=$row['periode']; $hr=$row['heure']; $ag=$row['agent'];
            $st1->bind_param('sssssssss',$ag,$ag,$d1,$f1,$tc,$tc,$dj,$pe,$hr);
            $st1->execute();
            $d=new DateTime($d1); $fi2=new DateTime($f1);
            while($d<=$fi2){$dts[]=$d->format('Y-m-d');$d->modify('+1 day');}
        }
        // Ajouter les jours retirés aux dates à rafraîchir
        $d=clone $splitDeb;
        while($d<=$splitFin){$dts[]=$d->format('Y-m-d');$d->modify('+1 day');}
        // INSERT plage 2 : lendemain de fin retiré → fin originale
        if ($prev2_deb <= $fin) {
            $d2 = $prev2_deb->format('Y-m-d'); $f2 = $fin->format('Y-m-d');
            $st2 = $mysqli->prepare("INSERT INTO conges (agent,agent_nom,date_debut,date_fin,type,type_conge,demi_jour,periode,heure) VALUES (?,?,?,?,?,?,?,?,?)");
            $tc=$row['type_conge']; $dj=$row['demi_jour']; $pe=$row['periode']; $hr=$row['heure']; $ag=$row['agent'];
            $st2->bind_param('sssssssss',$ag,$ag,$d2,$f2,$tc,$tc,$dj,$pe,$hr);
            $st2->execute();
            $d=new DateTime($d2); $fi2=new DateTime($f2);
            while($d<=$fi2){$dts[]=$d->format('Y-m-d');$d->modify('+1 day');}
        }
        $dts = array_unique($dts);
        $cells = renderCellsHtml($mysqli,[$row['agent']],$dts,$groupeAgent,$agentsTir,$agentsPermanence,$isAdmin,$globalLocked,$locks,$userAgents);
        echo json_encode(['ok'=>true,'cells'=>$cells,'msg'=>'Jour retiré de la plage']);
    ...
    /* ── Suppression CONGÉ ── */
        ...
        while($d<=$fi){$dts[]=$d->format('Y-m-d');$d->modify('+1 day');}
        // Rafraîchir aussi les autres agents du même groupe (cycle partagé)
        $grpAgentDel=$groupeAgent[$agent]??'';
        $agentsToRefreshDel=[$agent];
        if(in_array($grpAgentDel,['nuit','equipe','gie','gie_j'])){
            foreach($groupeAgent as $ag=>$grp){
                if($grp===$grpAgentDel && $ag!==$agent) $agentsToRefreshDel[]=$ag;
            }
            // Élargir au mois complet car le cycle affecte toutes les cellules
            $moisRefDel=substr($del_debut,0,7);
            $dts=[];
            $dm=new DateTime($moisRefDel.'-01');
            $fm=new DateTime($moisRefDel.'-'.date('t',strtotime($moisRefDel.'-01')));
            while($dm<=$fm){$dts[]=$dm->format('Y-m-d');$dm->modify('+1 day');}
        }
        $cells=$ok?renderCellsHtml($mysqli,$agentsToRefreshDel,$dts,$groupeAgent,$agentsTir,$agentsPermanence,$isAdmin,$globalLocked,$locks,$userAgents):[];
        $r1=$mysqli->query("SELECT MAX(id) as m FROM permanences");
        $r2=$mysqli->query("SELECT MAX(id) as m FROM conges");
        $m1=($r1&&$row=$r1->fetch_assoc())?(int)$row['m']:0;
        $m2=($r2&&$row=$r2->fetch_assoc())?(int)$row['m']:0;
    ...
    /* ── Recherche congés ── */
        ...
        $res=$mysqli->query("SELECT id,nom,prenom,tel_domicile,tel_gsm,tel_neo,fonction,groupe,pays FROM plan_rappel ORDER BY groupe,nom,prenom");
        $rows=[];
        while($r=$res->fetch_assoc()) $rows[]=$r;
        echo json_encode(['ok'=>true,'rows'=>$rows]);
        ...
        $pays=trim($_POST['pays']??'')==='BE'?'BE':'FR';
        if(!$nom){ echo json_encode(['ok'=>false,'msg'=>'Nom requis']); exit; }
        if($rid>0){
            $st=$mysqli->prepare("UPDATE plan_rappel SET nom=?,prenom=?,tel_domicile=?,tel_gsm=?,tel_neo=?,fonction=?,groupe=?,pays=? WHERE id=?");
            $st->bind_param('ssssssssi',$nom,$prenom,$dom,$gsm,$neo,$fct,$grp,$pays,$rid);
        } else {
            $st=$mysqli->prepare("INSERT INTO plan_rappel (nom,prenom,tel_domicile,tel_gsm,tel_neo,fonction,groupe,pays) VALUES (?,?,?,?,?,?,?,?)");
            $st->bind_param('ssssssss',$nom,$prenom,$dom,$gsm,$neo,$fct,$grp,$pays);
        }
        $ok=$st->execute();
...
function loadConges(mysqli $db,int $annee,int $moisD,int $moisF): array {
    $conges=[]; $congesIds=[]; $congesMeta=[]; $congesAll=[];
    ...
    while ($c=$res->fetch_assoc()) {
        ...
        while ($d<=$fi) {
            $ds=$d->format('Y-m-d');
            $entry = ['periode'=>$c['periode']??'J','libelle'=>$c['libelle'],'heure'=>$c['heure']??'','couleur_bg'=>$c['couleur_bg']??'#1565c0','couleur_txt'=>$c['couleur_txt']??'#ffffff','valide'=>(int)($c['valide']??0),'type_conge'=>$c['type_conge'],'id'=>$c['id'],'date_debut'=>$c['date_debut'],'date_fin'=>$c['date_fin']];
            if (!isset($congesMeta[$ag][$ds])) {
                $conges[$ag][$ds]    = $c['type_conge'];
                $congesIds[$ag][$ds] = $c['id'];
                $congesMeta[$ag][$ds]= $entry;
                $congesAll[$ag][$ds] = [$entry];
            } else {
                $congesAll[$ag][$ds][] = $entry;
                if (($entry['periode']??'J') === 'M') {
                    $conges[$ag][$ds]    = $c['type_conge'];
    ...
    return [$conges,$congesIds,$congesMeta,$congesAll];
}
...
[$conges,$congesIds,$congesMeta,$congesAll]           = loadConges($mysqli,$annee,$mois,$mois);
...
function ligne(string $agent,callable $cb,bool $masqueRC,bool $ignFe=false,string $grp='standard'): void {
  global $annee,$mois,$moisStr,$nbJours,$conges,$congesIds,$congesMeta,$congesAll,$perms,$permsIds,$feries,$agentsPermanence,$tirs,$tirsIds,$tirsAnnul,$agentsTir,$isAdmin,$globalLocked,$locks,$userAgents,$symSoleil,$svgIndispo,$vacOvr,$vacOvrId;
  ...
  for ($d=1;$d<=$nbJours;$d++) {
    ...
    if ($tirPer !== null) {
      ...
      $tip = match($pmData['type']){'M'=>'Permanence Matin','AM'=>'Permanence Après-midi','IM'=>'Indisponibilité Matin','IAM'=>'Indisponibilité Après-midi','IJ'=>'Indisponibilité Journée','P'=>'Permission','R'=>'Repos',default=>$pmData['type']};
      $pmType     = trim($pmData['type']);
      ...
      echo "<td class='$cls perm-ok$permLocked2$lockCls'$da data-ferie='0' data-conge-id='0' data-conge-type='' data-perm-id='$pmId' data-perm-type='$pmType' data-cycle-orig='$pmCycOrig' data-tir-id='0'>$dispPerm$sym</td>";
      ...
      $hre   = $meta['heure']??'';
      // Détection double congé (M + AM sur même jour)
      $allCgsL = $congesAll[$agent][$date] ?? [$meta + ['type_conge'=>$cgCode]];
      $isDoubleL = count($allCgsL) >= 2 && $cycleVal === 'J';
      if ($isDoubleL) {
          usort($allCgsL, fn($a,$b) => ($a['periode']==='M' ? 0 : 1) - ($b['periode']==='M' ? 0 : 1));
          $cg1L = $allCgsL[0]; $cg2L = $allCgsL[1];
          $code1L = $cg1L['type_conge']??$cgCode; $code2L = $cg2L['type_conge']??$cgCode;
          $lib1L  = $cg1L['libelle'] ?: $code1L;  $lib2L  = $cg2L['libelle'] ?: $code2L;
          $cellTxt = "$code1L+$code2L";
          $tip   = "½ $lib1L (Matin) + ½ $lib2L (Après-midi)";
          $sym   = ''; $hre = '';
          $id2L = $allCgsL[1]['id'] ?? 0;
          $dataConge2L = " data-conge2-type='".htmlspecialchars($code2L)."' data-conge2-per='AM' data-conge2-id='$id2L'";
          $tip = ''; // title= vide : le cell-tip JS gère l'affichage
      } else {
          $dataConge2L = '';
          $sym   = match($per){'M'=>'☀','AM'=>'🌙',default=>''};
          $cellTxt = $cgCode.$sym; // heure retirée de la cellule pour DA/PR
          $tip   = $lib;
          if($hre && in_array($cgCode,['DA','PR'])) $tip .= ' à '.substr($hre,0,5); // heure dans tooltip
          if($per!=='J') $tip .= ' ('.match($per){'M'=>'Matin','AM'=>'Après-midi',default=>$per}.')';
      }
      ...
      $prevData2 = ($cgCode==='PREV') ? " data-prev-debut='".htmlspecialchars($meta['date_debut']??$date)."' data-prev-fin='".htmlspecialchars($meta['date_fin']??$date)."'" : '';
      echo "<td class='$clsCg2$lockCls$valideCls2'$da$styleCg2 data-ferie='0' data-conge-id='$cgId' data-conge-type='$cgCode' data-conge-valide='$valide2' data-perm-id='0' data-tir-id='0' data-conge-per='$per' data-conge-heure='".htmlspecialchars($hre)."'$dataConge2L$prevData2>$cellTxt</td>";
    ...
    <div class="modal-body">
      ...
      <div id="sec-conge">
        ...
        <div id="sec-periode" style="display:none">
          ...
          <div class="options-row" id="options-periode">
            <!-- boutons générés dynamiquement par buildPeriode() -->
          </div>
        </div>
      </div>
      ...
      <div id="sec-tir" style="display:none">
        ...
        <div class="perm-grid" id="tir-grid">
          ...
          <button type="button" class="perm-btn" id="tir-j"    style="display:none;background:#1565c0;color:#ffd600">&#127919; TIR</button>
      ...
      <!-- Section scission PREV -->
      <div id="sec-split-prev" style="display:none;margin:8px 0;padding:8px;background:#fff8e1;border:1.5px solid #fb8c00;border-radius:7px">
        <div style="font-size:.75rem;font-weight:700;color:#e65100;margin-bottom:6px">✂️ Retirer des jours de la plage PREV</div>
        <div style="display:flex;gap:6px;align-items:center;margin-bottom:6px">
          <div style="flex:1"><label style="font-size:.68rem;color:#666">Du</label><br>
            <input type="date" id="split-debut" style="width:100%;padding:3px 5px;border:1.5px solid #fb8c00;border-radius:4px;font-size:.75rem"></div>
          <div style="flex:1"><label style="font-size:.68rem;color:#666">Au</label><br>
            <input type="date" id="split-fin" style="width:100%;padding:3px 5px;border:1.5px solid #fb8c00;border-radius:4px;font-size:.75rem"></div>
        </div>
        <button id="btn-split-prev" style="padding:5px 14px;background:#fb8c00;color:#fff;border:none;border-radius:5px;font-size:.78rem;font-weight:700;cursor:pointer">✂️ Retirer cette plage</button>
        <div id="split-confirm-band" style="display:none;margin-top:8px;padding:7px 10px;background:#c0392b;border-radius:5px;color:#fff;font-size:.75rem">
          <div style="font-weight:700;margin-bottom:5px">⚠️ Confirmer la suppression de cette plage ?</div>
          <div style="display:flex;gap:6px">
            <button id="btn-split-confirm" style="flex:1;padding:4px;background:#fff;color:#c0392b;border:none;border-radius:4px;font-weight:700;font-size:.75rem;cursor:pointer">✅ Confirmer</button>
            <button id="btn-split-cancel" style="flex:1;padding:4px;background:rgba(255,255,255,0.2);color:#fff;border:1px solid rgba(255,255,255,0.5);border-radius:4px;font-size:.75rem;cursor:pointer">✕ Annuler</button>
          </div>
        </div>
      </div>
      <!-- Section suppression double congé -->
      <div id="sec-double-del" style="display:none;margin:8px 0;padding:8px;background:#fff3f3;border:1.5px solid #e74c3c;border-radius:7px">
        <div style="font-size:.75rem;font-weight:700;color:#c0392b;margin-bottom:6px">🗑️ Quel congé supprimer ?</div>
        <div style="display:flex;gap:6px;flex-wrap:wrap">
          <button id="btn-del-matin"  style="flex:1;padding:5px 8px;background:#e74c3c;color:#fff;border:none;border-radius:5px;font-size:.75rem;font-weight:700;cursor:pointer">Supprimer Matin</button>
          <button id="btn-del-am"     style="flex:1;padding:5px 8px;background:#e74c3c;color:#fff;border:none;border-radius:5px;font-size:.75rem;font-weight:700;cursor:pointer">Supprimer Après-midi</button>
          <button id="btn-del-both"   style="flex:1;padding:5px 8px;background:#922b21;color:#fff;border:none;border-radius:5px;font-size:.75rem;font-weight:700;cursor:pointer">Supprimer les deux</button>
        </div>
      </div>
...
const AGENTS_TIR_GENDARMERIE=<?=json_encode($agentsTirGendarmerie,JSON_UNESCAPED_UNICODE)?>;
...
const AGENTS_TIR_COMBINE=['CoGe ROUSSEL','Cne MOKADEM','GP DHALLEWYN','BC DELCROIX','BC DRUEZ','LCL PARENT','ADJ CORRARD','ADJ LEFEBVRE','ADC LAMBERT'];
...
const CAN_TIR=<?=($userRights['can_tir']??false)?'true':'false'?>;
...
let curTd=null,selType='',selPer='J',curCongeId=0,curPermId=0,curTirId=0,curAnnulId=0,curPerExistante='',curCongeIdMatin=0,curCongeIdAM=0,
...
function openModal(td){
  ...
  // Exception : CoGe ROUSSEL, douane, et agents GIE/Gendarmerie sur leur propre ligne
  const isGieOuGendarmerie=(td.dataset.groupe==='gie'||td.dataset.groupe==='gie_j'||td.dataset.groupe==='lcl_parent'||td.dataset.groupe==='adc_lambert');
  const isOwnLine=USER_AGENTS.length>0&&USER_AGENTS.includes(td.dataset.agent);
  if(!IS_ADMIN && !IS_COGE && !IS_DOUANE && !(isGieOuGendarmerie&&isOwnLine) && curCongeId>0){
  ...
  selPer    ='J';
  curPerExistante='';
  curCongeIdMatin=0;curCongeIdAM=0;
  ...
  // Exception : congé demi-journée (M ou AM) sur cycle J → permettre aussi d'ajouter le congé complémentaire
  if(curCongeId>0){
    const congePerExist=td.dataset.congePer||'J';
    const isDemiConge=(congePerExist==='M'||congePerExist==='AM');
    const isCycleJ=(cycleVal==='J');
    const hasConge2=!!(td.dataset.conge2Type);

    if(isDemiConge && isCycleJ){
      if(hasConge2){
        // ── CAS 1 : DEUX demi-congés présents → mode gestion double ──
        const id1=parseInt(td.dataset.congeId)||0;
        const id2=parseInt(td.dataset.conge2Id)||0;
        curCongeIdMatin = congePerExist==='M' ? id1 : id2;
        curCongeIdAM    = congePerExist==='M' ? id2 : id1;
        // Pas de changement de curCongeId : le modal affiche la section suppression
        // Le bouton Supprimer standard est masqué (géré dans openCongeModal)
        curPerExistante=''; // pas de grisage nécessaire
      } else {
        // ── CAS 2 : UN SEUL demi-congé → permettre suppression ET ajout du complémentaire ──
        const perCompl=congePerExist==='M'?'AM':'M';
        selPer=perCompl;
        curPerExistante=congePerExist; // griser la période déjà posée
        // Stocker l'ID existant pour btn-del, forcer curCongeId=0 pour que Save fasse un INSERT
        const idExistant=parseInt(td.dataset.congeId)||0;
        if(congePerExist==='M'){ curCongeIdMatin=idExistant; curCongeIdAM=0; }
        else                   { curCongeIdAM=idExistant;    curCongeIdMatin=0; }
        curCongeId=0; // INSERT du complémentaire, pas UPDATE de l'existant
      }
    }
    modeTir=false;modePerm=false;openCongeModal(td,date,groupe,cycle,false);return;
  }
...
function openCongeModal(td,date,groupe,cycle,isPerm){
  ...
  ensureQuotaCache(); // Précharger quota en arrière-plan
    ...
    // En mode complémentaire (curPerExistante défini), selPer a déjà été fixé — ne pas l'écraser
    if(!curPerExistante){
    ...
    // Section suppression double congé
    const secDoubleDel=document.getElementById('sec-double-del');
    const isDoubleConge=!!(td.dataset.conge2Type);
    if(secDoubleDel) secDoubleDel.style.display=isDoubleConge?'block':'none';
    // Section scission PREV : visible si congé PREV sur plage > 1 jour
    const secSplit=document.getElementById('sec-split-prev');
    if(secSplit){
      const isPrev=(td.dataset.congeType==='PREV');
      const dateDebPrev=td.dataset.prevDebut||'';
      const dateFinPrev=td.dataset.prevFin||'';
      const isPlage=dateDebPrev && dateFinPrev && dateDebPrev!==dateFinPrev;
      secSplit.style.display=(isPrev&&isPlage)?'block':'none';
      // Stocker l'ID réel du congé PREV pour le split (indépendant de curCongeId)
      if(isPrev&&isPlage){
        secSplit.dataset.congeId=td.dataset.congeId||'0';
        // Pré-remplir avec la date cliquée
        const inpDeb=document.getElementById('split-debut');
        const inpFin=document.getElementById('split-fin');
        if(inpDeb){ inpDeb.value=td.dataset.date; inpDeb.min=dateDebPrev; inpDeb.max=dateFinPrev; }
        if(inpFin){ inpFin.value=td.dataset.date; inpFin.min=dateDebPrev; inpFin.max=dateFinPrev; }
        // Masquer les champs Début/Fin du modal (redondants avec Du/Au)
        const drRow=document.querySelector('.date-range-row');
        const rInfo=document.getElementById('range-info');
        if(drRow) drRow.style.display='none';
        if(rInfo) rInfo.style.display='none';
      } else {
        // Restaurer les champs Début/Fin si pas en mode split
        const drRow=document.querySelector('.date-range-row');
        if(drRow) drRow.style.display='';
      }
    }
    buildGrid(groupe);buildPeriode();
    // En mode complémentaire : forcer la sélection visuelle + message indicatif
    if(curPerExistante){
      const container=document.getElementById('options-periode');
      if(container){
        container.querySelectorAll('.opt-btn').forEach(b=>{
          const isSel=b.dataset.val===selPer;
          b.classList.toggle('sel',isSel);
          if(isSel){ b.style.background='#1a2742'; b.style.color='#fff'; }
          else      { b.style.background=b._bgOrig||''; b.style.color=b._txtOrig||''; }
        });
        // Message indicatif
        let hint=container.parentElement.querySelector('.compl-hint');
        if(!hint){
          hint=document.createElement('div');
          hint.className='compl-hint';
          hint.style.cssText='font-size:.72rem;color:#1565c0;margin-top:4px;font-style:italic';
          container.parentElement.appendChild(hint);
        }
        const perLbl=selPer==='M'?'Matin':'Après-midi';
        const existLbl=curPerExistante==='M'?'Matin':'Après-midi';
        hint.textContent='ℹ️ '+existLbl+' déjà posé — période '+perLbl+' pré-sélectionnée pour le 2ᵉ demi-congé';
      }
    } else {
      // Nettoyer le hint si présent
      const container=document.getElementById('options-periode');
      if(container) container.parentElement.querySelector('.compl-hint')?.remove();
    }
    ...
    const isDoubleCongeModal=!!(curTd&&curTd.dataset.conge2Type);
    const idPourDel=curCongeId||curCongeIdMatin||curCongeIdAM;
    if(isDoubleCongeModal){
      btnDel.style.display='none'; btnDel.disabled=true;
    } else {
      btnDel.style.display=(!IS_ADMIN&&!IS_DOUANE&&idPourDel>0&&(!_userAllowed.includes(congeTypeExist)||congeValide))?'none':'';
      btnDel.disabled=idPourDel===0||!canDelConge;
    }
...
function openTirModal(td,date,groupe,cycle,hasTir){
  ...
  const isGendarme=AGENTS_TIR_GENDARMERIE.includes(td.dataset.agent);
  if(isGendarme){
    // Gendarmerie : un seul bouton TIR Journée, pas de choix M/AM/Nuit
    document.getElementById('tir-m').style.display='none';
    document.getElementById('tir-am').style.display='none';
    document.getElementById('tir-j').style.display='block';
    document.getElementById('tir-nuit').style.display='none';
    // Pré-sélectionner J automatiquement
    document.querySelectorAll('#sec-tir .perm-btn').forEach(b=>b.classList.remove('sel'));
    document.getElementById('tir-j').classList.add('sel');
    selPer='J';
    document.getElementById('sec-tir').querySelector('p').style.display='none';
  } else {
    document.getElementById('tir-m').style.display   =isNuit||isAM?'none':'block';
    document.getElementById('tir-am').style.display  =isNuit||isMatin?'none':'block';
    document.getElementById('tir-j').style.display   ='none';
    document.getElementById('tir-nuit').style.display=(isNuit&&!isGendarme)?'block':'none';
  }
...
function buildGrid(g){
  ...
  if(GROUPES_SPECIAUX.includes(g)){
    if(g==='lcl_parent'){
    ...
    } else if(g==='douane'||g==='douane_j'){
      // Douane : grille 6 colonnes avec séparateurs, calquée sur le modèle Police
      const DOUANE_CODES=[
        '__SEP_DOUANE_VAC__',
        'J','M','AM','__EMPTY__','__EMPTY__','__EMPTY__',
        '__SEP_DOUANE_CONGE__',
        'AEA','CA','CET','NC','RC','RH',
        '__SEP_DOUANE_MED__',
        'CM','GEM','__EMPTY__','__EMPTY__','__EMPTY__','__EMPTY__',
      ];
      const DOUANE_SEP_LABELS={
        '__SEP_DOUANE_VAC__':  {label:'🕐 Vacations',    color:'#1565c0'},
        '__SEP_DOUANE_CONGE__':{label:'📋 Congés',        color:'#2e7d32'},
        '__SEP_DOUANE_MED__':  {label:'🏥 Médical',       color:'#6a1b9a'},
      };
      const DOUANE_LABELS={
        'J':'Journée','M':'Matin','AM':'Après-midi',
        'AEA':'Autor. Abs.','CA':'Cong. Ann.','CET':'Épargne',
        'NC':'Non Coté','RC':'Repos Comp.','RH':'Repos Heb.',
        'CM':'Maladie','GEM':'Garde Enf. Mal.'
      };
      codeGrid.style.gridTemplateColumns='repeat(6,1fr)';
      document.querySelector('.modal').style.width='720px';
      // Reconstruire la grille avec séparateurs et __EMPTY__
      codeGrid.innerHTML='';
      DOUANE_CODES.forEach(code=>{
        if(DOUANE_SEP_LABELS[code]){
          const sep=document.createElement('div');
          sep.style.cssText='grid-column:span 6;background:'+DOUANE_SEP_LABELS[code].color+';color:#fff;font-size:.72rem;font-weight:700;padding:4px 8px;border-radius:4px;margin-top:4px';
          sep.textContent=DOUANE_SEP_LABELS[code].label;
          codeGrid.appendChild(sep); return;
        }
        if(code==='__EMPTY__'){
          codeGrid.appendChild(document.createElement('div')); return;
        }
        const t=types.find(t=>t.code===code);
        if(!t) return;
        const btn=document.createElement('button');
        btn.type='button'; btn.dataset.code=t.code;
        btn.className='code-btn'+(selType===t.code?' sel':'');
        btn.style.cssText='background:'+t.couleur_bg+';color:'+t.couleur_txt+';padding:8px 4px;font-size:.78rem;min-height:48px';
        const lbl=DOUANE_LABELS[t.code]||'';
        btn.innerHTML='<strong>'+t.code+'</strong>'+(lbl?'<small style="display:block;font-size:.58rem;font-weight:400;margin-top:2px">'+lbl+'</small>':'');
        btn.title=t.libelle;
      ...
      return; // grille déjà construite manuellement
    } else {
...
function buildPeriode(){
  ...
  btns.forEach(b=>{
    ...
    // Mémoriser couleurs originales pour restauration au désel
    btn._bgOrig=btn.style.background||'';
    btn._txtOrig=btn.style.color||'';
    if(b.val===selPer){
      btn.classList.add('sel');
      btn.style.background='#1a2742';
      btn.style.color='#fff';
    }
    // En mode complémentaire : griser uniquement J et NUIT — M et AM restent switchwables
    if(curPerExistante && (b.val==='J' || b.val==='NUIT')){
      btn.disabled=true;
      btn.style.opacity='0.35';
      btn.title='Non disponible en mode complémentaire';
    }
    // Indiquer visuellement la période déjà posée (non cliquable pour la poser en double)
    if(curPerExistante && b.val===curPerExistante){
      btn.style.opacity='0.5';
      btn.title='Déjà posé — cliquez pour changer de demi-journée complémentaire';
    }
    btn.addEventListener('click',()=>{
      if(btn.disabled) return;
      selPer=b.val;
      // En mode complémentaire : si on clique l'autre demi-journée, inverser curPerExistante
      if(curPerExistante && (b.val==='M'||b.val==='AM')){
        curPerExistante=b.val==='M'?'AM':'M';
      }
      container.querySelectorAll('.opt-btn').forEach(x=>{
        x.classList.remove('sel');
        x.style.background=x._bgOrig||'';
        x.style.color=x._txtOrig||'';
        x.style.opacity='';
        x.title='';
        // Réappliquer le grisage selon le nouvel état
        if(curPerExistante && (x.dataset.val==='J'||x.dataset.val==='NUIT')){
          x.disabled=true; x.style.opacity='0.35';
        }
        if(curPerExistante && x.dataset.val===curPerExistante){
          x.style.opacity='0.5';
        }
      });
      btn.classList.add('sel');
...
Voulez-vous quand même enregistrer ?`);
      ...
      if(r.ok){
        ...
        setTimeout(silentRefreshAll, 300);
      } else {showToast('Erreur : '+r.msg,false);btnSave.disabled=false;}
    }
  }catch(e){showToast('Erreur réseau',false);btnSave.disabled=false;}
});

// Scission PREV : retirer un jour d'une plage
document.getElementById('btn-split-prev')?.addEventListener('click',async()=>{
  if(!curTd) return;
  const date=curTd.dataset.date;
  const deb=curTd.dataset.prevDebut;
  const fin=curTd.dataset.prevFin;
  const agent=curTd.dataset.agent;
  const splitId=parseInt(document.getElementById('sec-split-prev')?.dataset.congeId)||curCongeId;
  const splitDeb=document.getElementById('split-debut')?.value||date;
  const splitFin=document.getElementById('split-fin')?.value||date;
  if(splitFin<splitDeb){showToast('Date fin < date début',false);return;}
  // Afficher le bandeau de confirmation inline
  const band=document.getElementById('split-confirm-band');
  if(band) band.style.display='block';
  // Stocker les valeurs pour les listeners de confirmation
  band.dataset.splitId=splitId;
  band.dataset.splitDeb=splitDeb;
  band.dataset.splitFin=splitFin;
  band.dataset.agent=agent;
});
...
document.getElementById('btn-split-confirm')?.addEventListener('click',async()=>{
  ...
  const splitId=parseInt(band.dataset.splitId)||0;
  const splitDeb=band.dataset.splitDeb;
  const splitFin=band.dataset.splitFin;
  const agent=band.dataset.agent;
  try{
    const r=await ajax({action:'split_conge',conge_id:splitId,date_split:splitDeb,date_split_fin:splitFin,agent});
    if(r.ok){
      closeModal();
      applyCells(r);
      invalidateQuotaCache();
...
// Suppression double congé (matin / am / les deux)
async function deleteCongeById(id){
  if(!id||id===0) return true;
  const r=await ajax({action:'delete_conge',conge_id:id,agent:curTd.dataset.agent,date_debut:curTd.dataset.date});
  if(r.ok){ applyCells(r); invalidateQuotaCache(); }
  return r.ok;
}
document.getElementById('btn-del-matin')?.addEventListener('click',async()=>{
  if(!curTd||!confirm('Supprimer le congé Matin de '+curTd.dataset.agent+' ?'))return;
  const id=curCongeIdMatin||curCongeId;
  const ok=await deleteCongeById(id);
  if(ok){closeModal();showToast('Congé Matin supprimé');setTimeout(silentRefreshAll,300);}
  else showToast('Erreur suppression',false);
});
document.getElementById('btn-del-am')?.addEventListener('click',async()=>{
  if(!curTd||!confirm('Supprimer le congé Après-midi de '+curTd.dataset.agent+' ?'))return;
  const id=curCongeIdAM||curCongeId;
  const ok=await deleteCongeById(id);
  if(ok){closeModal();showToast('Congé Après-midi supprimé');setTimeout(silentRefreshAll,300);}
  else showToast('Erreur suppression',false);
});
document.getElementById('btn-del-both')?.addEventListener('click',async()=>{
  if(!curTd||!confirm('Supprimer les deux congés de '+curTd.dataset.agent+' ?'))return;
  const ok1=await deleteCongeById(curCongeIdMatin||curCongeId);
  const ok2=await deleteCongeById(curCongeIdAM);
  if(ok1||ok2){closeModal();showToast('Congés supprimés');setTimeout(silentRefreshAll,300);}
  else showToast('Erreur suppression',false);
});
...
btnDel.addEventListener('click',async()=>{
  ...
  try{
    const r=modePerm
      ...
      : await ajax({action:'delete_conge',conge_id:curCongeId||curCongeIdMatin||curCongeIdAM,agent:curTd.dataset.agent,date_debut:curTd.dataset.date});
    if(r.ok){
      ...
      // Forcer un refresh complet pour les groupes cycliques
      setTimeout(silentRefreshAll, 300);
...
async function loadRappel(){
  ...
  // Formate un numéro selon le pays et le type
  // Formate un numéro automatiquement selon son préfixe
  // +33 ou 06/07 → FR | +32 ou 04xx → BE mobile | +32 → BE fixe
  // neo=true → toujours FR (NEO est un réseau FR)
  const fmtTel=(v, _paysDef, _mobile, neo=false)=>{
    if(!v) return '<span style="display:block;text-align:center;color:#aaa">—</span>';
    let n=v.replace(/[\s.]/g,''); // supprimer espaces et points saisis
    // Détection 100% automatique depuis le préfixe
    // Préfixe international obligatoire pour formatage correct
    if(n.startsWith('+33')){
      // France : +33 + 9 chiffres
      const d=n.substring(3);
      const display='+33.'+d.substring(0,1)+'.'+d.substring(1,3)+'.'+d.substring(3,5)+'.'+d.substring(5,7)+'.'+d.substring(7,9);
      return `<a href="tel:+33${d}" style="color:#1565c0;font-weight:700;text-decoration:none;white-space:nowrap">${display.replace(/\.+$/,'')}</a>`;
    }
    if(!neo && n.startsWith('+32')){
      // Belgique : +32 + chiffres (sans 0 initial)
      let d=n.substring(3);
      if(d.startsWith('0')) d=d.substring(1);
      const isMobile=d.startsWith('4'); // 04xx = mobile BE
      let display;
      if(isMobile){
        // Mobile BE : +32.4XX.XX.XX.XX (9 chiffres)
        display='+32.'+d.substring(0,3)+'.'+d.substring(3,5)+'.'+d.substring(5,7)+'.'+d.substring(7,9);
      } else {
        // Fixe BE : +32.XX.XX.XX.XX (8 chiffres, ex: 56.33.31.10)
        display='+32.'+d.substring(0,2)+'.'+d.substring(2,4)+'.'+d.substring(4,6)+'.'+d.substring(6,8);
      }
      return `<a href="tel:+32${d}" style="color:#1565c0;font-weight:700;text-decoration:none;white-space:nowrap">🇧🇪 ${display.replace(/\.+$/,'')}</a>`;
    }
    if(neo && n.startsWith('+32')){
      // NEO toujours FR même si +32 saisi par erreur — traiter comme +33
      const d=n.substring(3);
      const display='+33.'+d.substring(0,1)+'.'+d.substring(1,3)+'.'+d.substring(3,5)+'.'+d.substring(5,7)+'.'+d.substring(7,9);
      return `<a href="tel:+33${d}" style="color:#1565c0;font-weight:700;text-decoration:none;white-space:nowrap">${display.replace(/\.+$/,'')}</a>`;
    }
    // Pas de préfixe international → affichage brut en orange pour signaler la saisie à corriger
    return `<span title="⚠️ Saisir avec +33 ou +32" style="color:#e67e22;font-weight:700;cursor:help">${v} ⚠️</span>`;
  };
  ...
  if(r.rows.length){
    ...
    visibleGroupes.forEach((grp,gi)=>{
      ...
      rows.forEach((row,i)=>{
        ...
        if(isFirst){
          html+=`<td rowspan="${rows.length}" style="padding:5px 8px;font-weight:400;font-size:.85rem;color:#fff;background:#1a2742;text-align:center;vertical-align:middle;border:3px solid #fff">${grp}</td>`;
        }
        html+=`
          <td style="${tdS};font-weight:800;color:#1a2742;border-top:${borderTop};border-bottom:${borderBot}">${row.nom}</td>
          <td style="${tdS};font-weight:700;color:#1a2742;border-top:${borderTop};border-bottom:${borderBot}">${row.prenom||'—'}</td>
          <td style="${tdS};border-top:${borderTop};border-bottom:${borderBot}">${fmtTel(row.tel_domicile)}</td>
          <td style="${tdS};border-top:${borderTop};border-bottom:${borderBot}">${fmtTel(row.tel_gsm)}</td>
          <td style="${tdS};border-top:${borderTop};border-bottom:${borderBot}">${fmtTel(row.tel_neo,'FR',false,true)}</td>
  ...
  if(IS_ADMIN){
    html+=`<div id="rappel-form" style="border:1.5px solid #e0e0e0;border-radius:8px;padding:12px;background:#f9f9f9;margin-top:8px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr 0.6fr 1fr 1fr 1fr 1fr auto;gap:8px;align-items:end">
        ...
        <div><label style="font-size:.72rem;color:#666">Domicile</label><br>
          <input id="rappel-dom" type="text" placeholder="+33.X.XX.XX.XX.XX ou +32.XX.XX.XX.XX" style="width:100%;padding:4px 6px;border:1.5px solid #ccc;border-radius:5px;font-size:.78rem"></div>
        <div><label style="font-size:.72rem;color:#666">GSM / Portable</label><br>
          <input id="rappel-gsm" type="text" placeholder="+33.X.XX.XX.XX.XX" style="width:100%;padding:4px 6px;border:1.5px solid #ccc;border-radius:5px;font-size:.78rem"></div>
        <div><label style="font-size:.72rem;color:#666">NEO 🇫🇷</label><br>
          <input id="rappel-neo" type="text" placeholder="+33.X.XX.XX.XX.XX" style="width:100%;padding:4px 6px;border:1.5px solid #ccc;border-radius:5px;font-size:.78rem"></div>
  ...
  document.getElementById('btn-print-rappel')?.addEventListener('click',()=>{
    const tblPrint=body.querySelector('table');
    if(!tblPrint){alert('Aucune donnée à imprimer.');return;}
    const clone=body.cloneNode(true);
    // Supprimer : boutons, formulaire, colonne Actions entière (th + td)
    clone.querySelectorAll('.btn-edit-rappel,.btn-del-rappel,#rappel-form,#btn-print-rappel').forEach(el=>el.remove());
    // Supprimer la colonne Actions (dernier th + tous les td qui la contiennent)
    const tblClone=clone.querySelector('table');
    if(tblClone){
      // Supprimer le dernier <th> (Actions)
      tblClone.querySelectorAll('thead tr th:last-child').forEach(th=>th.remove());
      // Supprimer le dernier <td> de chaque ligne — toujours, sans condition
      tblClone.querySelectorAll('tbody tr').forEach(tr=>{
        const last=tr.querySelector('td:last-child');
        if(last) last.remove();
      });
      // Supprimer le colgroup
      tblClone.querySelectorAll('colgroup').forEach(cg=>cg.remove());
    }
    clone.querySelectorAll('a[href^="tel:"]').forEach(a=>{a.style.color='#000';a.style.textDecoration='none';});
    // Retirer white-space:nowrap sur les td pour permettre le retour à la ligne si besoin
    clone.querySelectorAll('td,th').forEach(el=>{ el.style.whiteSpace='normal'; el.style.overflow='visible'; });
    ...
    fr.contentDocument.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Plan de rappel</title>
      <style>
        ...
        *{-webkit-print-color-adjust:exact;print-color-adjust:exact;box-sizing:border-box}
        body{font-family:Arial,sans-serif;margin:0;font-size:9.5px}
        table{border-collapse:collapse;width:100%;table-layout:auto}
        th,td{padding:3px 5px;border:1px solid #bbb;white-space:nowrap}
        th{background:#1a2742!important;color:#fff!important;font-size:9px;text-align:center}
        td[rowspan]{background:#1a2742!important;color:#fff!important;font-weight:800;text-align:center;vertical-align:middle}
        h1{font-size:12px;color:#1a2742;margin:0 0 5px}
        a{color:#000!important;text-decoration:none!important}
      </style>
    </head><body>
      <h1>📞 Plan de rappel — C.C.P.D. Tournai</h1>
      ${clone.innerHTML}
    </body></html>`);
    fr.contentDocument.close();
    setTimeout(()=>{fr.contentWindow.focus();fr.contentWindow.print();},300);
